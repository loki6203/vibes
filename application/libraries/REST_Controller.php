...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _detect_api_key()
    {
        ...
        $api_device_variable = $this->config->item('rest_device_name');
      ...
      $sess_device = 'HTTP_' . strtoupper(str_replace('-', '_', $api_device_variable));
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
			...
			$did = isset($this->_args[$api_device_variable]) ? $this->_args[$api_device_variable] : $this->input->server($sess_device);
        // echo $udd;    
        // echo $did;exit;     
	
            if (!($row=$this->rest->db->get_where($this->config->item('rest_keys_table'),
            array($this->config->item('rest_key_column')=>$key,
               $this->config->item('rest_sess_column')=>$udd,$this->config->item('rest_device_column')=>$did))->row()))
             {
                return FALSE;
