...
class Admin extends CI_Controller {
	...
	public function save_employee()
	{
		...
		$config = array( 
			...
			array( 'field'   => 'client_billable', 'label'   => 'Client Billable','rules'   => 'required'),
        ...
        else
        {
		        ...
		        if($_FILES['simage']['name'] !='')
		        {
		            ...
		            $this->check_file_type($imagename,'pdf','emp_identification_image');
	...
	public function export_employees()
	{
		...
		foreach($dt as $k=>$val){
			...
			$permanent_address=preg_replace('/\s+/', ' ', $val['permanent_address']);
			 $present_address=preg_replace('/\s+/', ' ', $val['present_address']);
			 $aa.=$counter."\t".$val['fname'].' '.$val['lname']."\t".$val['email_id']."\t".$val['personal_email_id']."\t".$val['gender']."\t".$val['dob']."\t".$val['emp_code']."\t".$designation."\t".$val['employment_type']."\t".$val['phone_no_code'].' '.$val['phone_no']."\t".$val['date_of_joining']."\t".$permanent_address."\t".$present_address."\t".$val['local_contact_name']."\t".$val['local_contact_relationship']."\t".$val['local_contact_ph_code'].' '.$val['local_contact_ph']."\t".$val['overseas_contact_name']."\t".$val['overseas_contact_relationship']."\t".$val['overseas_contact_ph_code'].' '.$val['overseas_contact_ph']."\t".$val['bank_name']."\t".$val['account_number']."\t".$val['account_type']."\t".$val['branch_name']."\t".$reporting_manager."\t".$hr_manager."\t".$lead_manager."\t".$val['client_manager']."\t".$client."\t".$identification_type."\t".$val['identification_number']."\t".$Status."\t".$val['comments']."\t".$val['termination_date']."\t".$role."\t".$val['leaves_included_in_contract']."\t".$val['client_billable']."\t".$val['created_at']."\n";
             $counter++;
        }
        echo $aa;
	}
	...
	public function save_other_documents()
	{
        ...
        else
        {
		        ...
		        if($_FILES['simage']['name']!='')
		        {
		            ...
		            $config['overwrite']=true;
		            $this->upload->initialize($config);
		            $this->check_file_type($imagepath,'pdf','other_documents');
		            if(!$this->upload->do_upload('simage'))
		            {
		                $this->session->set_flashdata('failed','<div class="alert alert-danger msgfade"><strong>Please upload jpg,jpeg.png formates only</strong></div>');
		                redirect($_SERVER['HTTP_REFERER']);
	...
	public function update_other_documents()
	{
        ...
        else
        {
		        ...
		        if($_FILES['simage']['name'] !='')
		        {
		            ...
		            $config['overwrite']=true;
		            $this->upload->initialize($config);
		            $this->check_file_type($imagename,'pdf','other_documents');
		            if(!$this->upload->do_upload('simage'))
		            {
	...
	public function edit_asset_BK($asset_id='')
	{
		checklogin_admin('Asset','Write');
		$admin_id=$this->session->userdata('emp_id');
	...
	public function update_asset_BK()
	{
		checklogin_admin();
		$admin_id=$this->session->userdata('emp_id');
	...
	public function change_asset_BK_status($asset_id='',$sta='')
	{
		checklogin_admin('Asset','Write');
...
// 		echo $this->db->last_query();exit;
	...
	public function save_employee_documents()
	{
        ...
        else
        {
		        ...
		        foreach ($_FILES['itcimage']['name'] as $key => $image)
		        {
			            ...
			            $this->load->library('upload');
			            $this->upload->initialize($config);
			            $this->check_file_type($filePath,'pdf','employee_documents');
		        ...
		        foreach ($_FILES['criminalimage']['name'] as $key2 => $image2)
		        {
			            ...
			            $this->check_file_type($filePath2,'pdf','employee_documents');
		        ...
		        foreach ($_FILES['educationalimage']['name'] as $key3 => $image3)
		        {
			            ...
			            $this->check_file_type($filePath3,'pdf','employee_documents');
		        ...
		        foreach ($_FILES['referenceimage']['name'] as $key4 => $image4)
		        {
			            ...
			            $this->check_file_type($filePath4,'pdf','employee_documents');
	...
	public function update_employee_documents()
	{
        ...
        else
        {
		        ...
		        if($_FILES['simage']['name'] !='')
		        {
		            ...
		            $config['overwrite']=true;
		            $this->upload->initialize($config);
		            $this->check_file_type($imagename,'jpg','employee_documents');
		            if(!$this->upload->do_upload('simage'))
		            {
		                $this->session->set_flashdata('failed','<div class="alert alert-danger msgfade"><strong>Please upload jpg,jpeg.png formates only</strong></div>');
		                redirect($_SERVER['HTTP_REFERER']);
	...
	public function update_emp_individual_doc()
	{
        ...
        else
        {
		        ...
		        if($_FILES['simage']['name'] !='')
		        {
		            ...
		            $config['overwrite']=true;
		            $this->upload->initialize($config);
		            $this->check_file_type($imagename,'pdf','employee_documents');
		            if(!$this->upload->do_upload('simage'))
		            {
		                $this->session->set_flashdata('failed','<div class="alert alert-danger msgfade"><strong>Please upload jpg,jpeg.png formates only</strong></div>');
		                redirect($_SERVER['HTTP_REFERER']);
	...
	public function save_payslip()
	{
        ...
        $config['overwrite']=true;
        $this->upload->initialize($config);
        $this->check_file_type($image_path,'onlypdf','payslips');
       	if(!$this->upload->do_upload('simage'))
        {
	...
	{
		checklogin_admin('Freeze Timesheet');
		$admin_id=$this->session->userdata('emp_id');
	...
	public function job_position()
	{
		...
		$data['companies'] = $this->db->query("SELECT t1.`company_id`, t1.`company_name`, t1.`is_active`, t2.`job_id`, t2.`company_id` as job_c_id, t2.`job_title`, t2.`is_active` as `job_is_active` FROM `companies` as `t1` LEFT JOIN `jobs` as `t2` ON `t1`.`company_id` = `t2`.`company_id` WHERE t2.`job_title` IS NOT NULL GROUP BY t2.`company_id`")->result_array();
		$WHEre='';
		if($search!='' && $search!='NA' && $active_inactive!='' && $cmpny!=''){
			$WHEre="t2.`job_title` LIKE '%$search%' AND t2.`is_active`=$active_inactive AND t1.`company_id`=$cmpny";
		}else if($search=='' && $search!='NA' && $active_inactive=='' && $cmpny!=''){
			$WHEre="t1.`company_id`=$cmpny";
		}else if($search=='' && $search!='NA' && $active_inactive!='' && $cmpny==''){
			$WHEre="t2.`is_active`=$active_inactive";
		}else if($search=='' && $search!='NA' && $active_inactive!='' && $cmpny!=''){
			$WHEre="t2.`is_active`=$active_inactive AND t1.`company_id`=$cmpny";
		}else if($search!='' && $search!='NA' && $active_inactive=='' && $cmpny==''){
			$WHEre="t2.`job_title` LIKE '%$search%'";
		}else if($search!='' && $search!='NA' && $active_inactive!=''){
			$WHEre="t2.`job_title` LIKE '%$search%' AND t2.`is_active`=$active_inactive";
		}else if($search!='' && $search!='NA' && $cmpny!=''){
			$WHEre="t2.`job_title` LIKE '%$search%' AND t1.`company_id`=$cmpny";
		}else if($search!='' && $search=='NA' && $active_inactive!='' && $cmpny!=''){
			$WHEre="t2.`is_active`=$active_inactive AND t1.`company_id`=$cmpny GROUP BY t1.`company_id`";
		}else if($search!='' && $search=='NA' && $active_inactive=='' && $cmpny!=''){
			$WHEre="t1.`company_id`=$cmpny";
		}else if($search!='' && $search=='NA' && $active_inactive!=''){
			$WHEre="t2.`job_title` LIKE '%$search%' AND t2.`is_active`=$active_inactive";
		...
		if($search!='' || $type!='' || $active_inactive!='' || $cmpny!=''){
		    if($search!='' && $search!='NA' && $active_inactive=='' && $cmpny==''){
				...
				$data['job_position']=$this->db->query("SELECT t1.`company_id`, t1.`company_name`, t1.`is_active`, t2.`job_id`, t2.`company_id` as job_c_id, t2.`job_title`, t2.`is_active` as `job_is_active` FROM `companies` as `t1` LEFT JOIN `jobs` as `t2` ON `t1`.`company_id` = `t2`.`company_id` WHERE $WHEre AND t2.`job_title` IS NOT NULL")->result_array();
				$this->load->view('admin/ajax_job_position',$data);
			}else if($search=='NA' && $active_inactive=='' && $cmpny==''){
	...
	public function preview_job_details()
	{
	    checklogin_admin('Job Positions');
	    $admin_id=$this->session->userdata('emp_id');
	    $job_id = $this->input->post('job_id');
	    $jobArr=$this->db->query("SELECT `job_id`, `company_id`, `job_title`, `department`, `location`, `remote_in_career_page`, `job_description`, `employment_type`, `seniority_level`, `industry_type`, `salary_range`, `minimum`, `maximum`, `work_experience_from`, `work_experience_to`, `number_of_openings`, `skills`, `education`, `is_active`, `created_at`, `updated_at` FROM `jobs` WHERE `job_id`=$job_id")->row_array();
	...
	public function change_job_status()
	{
	    checklogin_admin('Job Positions');
	    $admin_id=$this->session->userdata('emp_id');
		$job_id = $this->input->post('data_value');
	...
	public function candidate_status($company_id='',$job_id='')
	{
		...
		if($search!='' && $active_inactive=='NA')
		{
			...
			if($search!='' && $search!='NA'){
				...
				$data['Arr_interview']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id AND `status`='internal_interview' $WHere")->result_array();
				$data['internal_interview']=$this->load->view('admin/internal_interview',$data,TRUE);
				$data['Arr_presented']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id AND `status`='presented' $WHere")->result_array();
				$data['presented']=$this->load->view('admin/presented',$data,TRUE);
				$data['Arr_shortlisted']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id AND `status`='shortlisted' $WHere")->result_array();
				$data['shortlisted']=$this->load->view('admin/shortlisted',$data,TRUE);
				$data['Arr_client_interview']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id AND `status`='client_interview' $WHere")->result_array();
				$data['client_interview']=$this->load->view('admin/client_interview',$data,TRUE);
		...
		}else if($active_inactive!='' && $active_inactive!='NA' && ($search=='' || $search=='NA')){
			...
			if($active_inactive=='source/applied')
			{
				...
				$data['internal_interview']=$this->load->view('admin/internal_interview',$data,TRUE);
				$data['Arr_presented']=array();
				$data['presented']=$this->load->view('admin/presented',$data,TRUE);
				$data['Arr_shortlisted']=array();
				$data['shortlisted']=$this->load->view('admin/shortlisted',$data,TRUE);
				$data['Arr_client_interview']=array();
				$data['client_interview']=$this->load->view('admin/client_interview',$data,TRUE);
				...
				$data['Arr_rejected']=array();
				$data['rejected']=$this->load->view('admin/rejected',$data,TRUE);
			}else if($active_inactive=='internal_interview'){
				$data['Arr_interview']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id AND `status`='internal_interview'")->result_array();
				$data['internal_interview']=$this->load->view('admin/internal_interview',$data,TRUE);
				$data['Arr_source']=array();
				$data['source_applied']=$this->load->view('admin/source_applied',$data,TRUE);
				$data['Arr_contacted']=array();
				...
				$data['Arr_rejected']=array();
				$data['rejected']=$this->load->view('admin/rejected',$data,TRUE);
			}else if($active_inactive=='presented'){
				$data['Arr_source']=array();
				$data['source_applied']=$this->load->view('admin/source_applied',$data,TRUE);
				$data['Arr_contacted']=array();
				$data['contacted']=$this->load->view('admin/contacted',$data,TRUE);
				$data['Arr_interview']=array();
				$data['internal_interview']=$this->load->view('admin/internal_interview',$data,TRUE);
				$data['Arr_presented']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id AND `status`='presented'")->result_array();
				$data['presented']=$this->load->view('admin/presented',$data,TRUE);
				$data['Arr_shortlisted']=array();
				$data['shortlisted']=$this->load->view('admin/shortlisted',$data,TRUE);
				$data['Arr_client_interview']=array();
				...
				$data['Arr_rejected']=array();
				$data['rejected']=$this->load->view('admin/rejected',$data,TRUE);
			}else if($active_inactive=='shortlisted'){
				$data['Arr_source']=array();
				$data['source_applied']=$this->load->view('admin/source_applied',$data,TRUE);
				$data['Arr_contacted']=array();
				$data['contacted']=$this->load->view('admin/contacted',$data,TRUE);
				...
				$data['Arr_presented']=array();
				$data['presented']=$this->load->view('admin/presented',$data,TRUE);
				$data['Arr_shortlisted']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id AND `status`='shortlisted'")->result_array();
				$data['shortlisted']=$this->load->view('admin/shortlisted',$data,TRUE);
				$data['Arr_client_interview']=array();
				$data['client_interview']=$this->load->view('admin/client_interview',$data,TRUE);
				$data['Arr_hired']=array();
				...
				$data['Arr_rejected']=array();
				$data['rejected']=$this->load->view('admin/rejected',$data,TRUE);
			}else if($active_inactive=='client_interview'){
				$data['Arr_source']=array();
				$data['source_applied']=$this->load->view('admin/source_applied',$data,TRUE);
				$data['Arr_contacted']=array();
				$data['contacted']=$this->load->view('admin/contacted',$data,TRUE);
				...
				$data['internal_interview']=$this->load->view('admin/internal_interview',$data,TRUE);
				$data['Arr_presented']=array();
				$data['presented']=array();
				$data['Arr_shortlisted']=array();
				$data['shortlisted']=$this->load->view('admin/shortlisted',$data,TRUE);
				$data['Arr_client_interview']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id AND `status`='client_interview'")->result_array();
				$data['client_interview']=$this->load->view('admin/client_interview',$data,TRUE);
				$data['Arr_hired']=array();
				$data['hired']=$this->load->view('admin/hired',$data,TRUE);
				$data['Arr_rejected']=array();
				...
				echo(json_encode($data));
		}else if($search!='' && $active_inactive!='' && $active_inactive!='NA'){
			...
			if($active_inactive=='source/applied')
			{
				...
				$data['Arr_interview']=array();
				$data['internal_interview']=$this->load->view('admin/internal_interview',$data,TRUE);
				$data['Arr_presented']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id $WHere")->result_array();
				$data['presented']=$this->load->view('admin/presented',$data,TRUE);
				$data['Arr_shortlisted']=array();
				$data['shortlisted']=$this->load->view('admin/shortlisted',$data,TRUE);
				$data['Arr_client_interview']=array();
				...
				$data['Arr_presented']=array();
				$data['presented']=$this->load->view('admin/presented',$data,TRUE);
				$data['Arr_shortlisted']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id $WHere")->result_array();
				$data['shortlisted']=$this->load->view('admin/shortlisted',$data,TRUE);
				$data['Arr_client_interview']=array();
				$data['client_interview']=$this->load->view('admin/client_interview',$data,TRUE);
				$data['Arr_hired']=array();
				...
				$data['Arr_shortlisted']=array();
				$data['shortlisted']=$this->load->view('admin/shortlisted',$data,TRUE);
				$data['Arr_client_interview']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id $WHere")->result_array();
				$data['client_interview']=$this->load->view('admin/client_interview',$data,TRUE);
				$data['Arr_hired']=array();
				$data['hired']=$this->load->view('admin/hired',$data,TRUE);
				$data['Arr_rejected']=array();
				...
				echo(json_encode($data));
		}else if($search=='' && $active_inactive=='NA'){
			$data['Arr_source']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id AND `status`='source/applied'")->result_array();
			$data['source_applied']=$this->load->view('admin/source_applied',$data,TRUE);
			$data['Arr_contacted']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id AND `status`='contacted'")->result_array();
			$data['contacted']=$this->load->view('admin/contacted',$data,TRUE);
			$data['Arr_interview']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id AND `status`='internal_interview'")->result_array();
			$data['internal_interview']=$this->load->view('admin/internal_interview',$data,TRUE);
			$data['Arr_presented']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id AND `status`='presented'")->result_array();
				$data['presented']=$this->load->view('admin/presented',$data,TRUE);
				$data['Arr_shortlisted']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id AND `status`='shortlisted'")->result_array();
			...
			$data['Arr_hired']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id AND `status`='hired'")->result_array();
			$data['hired']=$this->load->view('admin/hired',$data,TRUE);
			$data['Arr_rejected']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$s_company_id AND `job_id`=$s_job_id AND `status`='rejected'")->result_array();
			$data['rejected']=$this->load->view('admin/rejected',$data,TRUE);
			echo(json_encode($data));
		}else{
			...
			$data['contacted']=$this->load->view('admin/contacted',$data,TRUE);
			$data['Arr_interview']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$company_id AND `job_id`=$job_id AND `status`='internal_interview'")->result_array();
			$data['internal_interview']=$this->load->view('admin/internal_interview',$data,TRUE);
			$data['Arr_presented']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$company_id AND `job_id`=$job_id AND `status`='presented'")->result_array();
			$data['presented']=$this->load->view('admin/presented',$data,TRUE);
			$data['Arr_shortlisted']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$company_id AND `job_id`=$job_id AND `status`='shortlisted'")->result_array();
			$data['shortlisted']=$this->load->view('admin/shortlisted',$data,TRUE);
			$data['Arr_client_interview']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$company_id AND `job_id`=$job_id AND `status`='client_interview'")->result_array();
			$data['client_interview']=$this->load->view('admin/client_interview',$data,TRUE);
	...
	public function ajax_candidate_status()
	{
		...
		$LogsArr=array('emp_id'=>$emp_id,'candidate_applied_id'=>$candidate_applied_id,'company_id'=>$company_id,'job_id'=>$job_id,'status'=>$value,'date'=>date("Y-m-d H:i:s"));
		$this->db->insert('candidate_job_status_logs',$LogsArr);
		if($val=='source_applied'){
			$data['Arr_source']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$company_id AND `job_id`=$job_id AND `status`='source/applied'")->result_array();
			...
			$data['Arr_interview']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$company_id AND `job_id`=$job_id AND `status`='internal_interview'")->result_array();
		}else if($val=='presented'){
			$data['Arr_presented']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$company_id AND `job_id`=$job_id AND `status`='$val'")->result_array();
		}else if($val=='shortlisted'){
			$data['Arr_shortlisted']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$company_id AND `job_id`=$job_id AND `status`='$val'")->result_array();
		}else if($val=='client interview'){
			$data['Arr_client_interview']=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `user_photo_name`, `user_photo_path`, `work_experience`, `rating`, `status`, `created_at` FROM `candidate_applied_jobs` WHERE `company_id`=$company_id AND `job_id`=$job_id AND `status`='client_interview'")->result_array();
		}else if($val=='hired'){
		...
		if($val=='source_applied'){
			...
			echo $this->load->view('admin/internal_interview',$data,TRUE);
		}else if($val=='presented'){
			echo $this->load->view('admin/presented',$data,TRUE);
		}else if($val=='shortlisted'){
			echo $this->load->view('admin/shortlisted',$data,TRUE);
		}else if($val=='client interview'){
			echo $this->load->view('admin/client_interview',$data,TRUE);
		}else if($val=='hired'){
	...
	public function ajax_candidate_comments()
	{
		checklogin_admin();
		$admin_id=$this->session->userdata('emp_id');
		$Getval=$this->input->post('val');
		$candidate_applied_id=$this->input->post('candidate_applied_id');
		if($Getval==''){
			$GetCompanyJobIds=$this->db->get_where('candidate_applied_jobs', array('candidate_applied_id='=>$candidate_applied_id))->row_array();
			$val=$GetCompanyJobIds['status'];
		}else{
			$val=$this->input->post('val');
		}
		$company_id=$this->input->post('company_id');
		$job_id=$this->input->post('job_id');
		$cmnts=$this->input->post('cmnts');
		if($val=='source_applied'){
			$value='source/applied';
		}else if($val=='internal interview'){
			$value='internal_interview';
		...
		if($cmnts!='')
		{
			...
			$LogsArr=array('emp_id'=>$emp_id,'candidate_applied_id'=>$candidate_applied_id,'company_id'=>$company_id,'job_id'=>$job_id,'status'=>$value,'comments'=>$cmnts,'date'=>date("Y-m-d H:i:s"));
			$res=$this->db->insert('candidate_job_status_logs',$LogsArr);
		}
		$data['comments']=$this->db->query("SELECT `candidate_job_status_logs_id`, `emp_id`, `candidate_applied_id`, `company_id`, `job_id`, `status`, `date`, `comments` FROM `candidate_job_status_logs` WHERE `candidate_applied_id`=$candidate_applied_id ORDER BY `candidate_job_status_logs_id` DESC")->result_array();
		echo $this->load->view('admin/ajax_candidate_comments',$data,TRUE);
	}
	public function export_candidate_application($type='',$company_id='',$job_id='')
	{
		...
		if(!empty($dt)){
			$j2_id=$dt[0]['job_id'];
				if($j2_id!=''){
					...
					$name ='All_Candidate_Profiles_'.$date;
				}
		}else{
			$name = 'All_Candidate_Profiles_'.$date;
		}
        ...
        header("Content-Type: application/vnd.ms-excel");
		header("Content-disposition: Attachment; filename=$name");
		$aa ="S.No\tCompany Name\tJob Title\tCandidate Name\tEmail Id\tPhone No\tGender\tDate Of Birth\tAddress\tPostcode\tCountry\tState/Province\tCity\tPassport No\tCurrent CTC\tExpected CTC\tSkills\tWork link/Portfolio\tNotice Period\tStatus\tComments\n";
		$counter=1;
		foreach($dt as $k=>$val){
			$c_id=$val['company_id'];
			if($val['company_id']!=''){
			...
			if($val['status']=='internal_interview'){
			    $status="internal interview";
			}else if($val['status']=='client_interview'){
			    $status="client interview";
			}else{
			    $status=$val['status'];
			}
			$aa.=$counter."\t".@$company_name['company_name']."\t".@$job_title['job_title']."\t".$val['fname'].' '.$val['lname']."\t".$val['email_id']."\t".$val['phone_no']."\t".$val['gender']."\t".$val['dob']."\t".$val['address']."\t".$val['postcode']."\t".$val['country']."\t".$val['state']."\t".$val['city']."\t".$val['id_passport_no']."\t".$val['ctc_from'].'-'.$val['ctc_to']."\t".$val['excepted_ctc_from'].'-'.$val['excepted_ctc_to']."\t".$val['skills']."\t".$val['work_link_portfolio']."\t".$val['notice_period']."\t".$status."\t".$val['comments']."\n";
             $counter++;
        }
        echo $aa;
	}
  ...
  //       $job_id = $this->encryption->decrypt(str_replace(array('-', '_', '~'), array('+', '/', '='), $j_id));
		...
		$config = array( 
			...
			if ($this->form_validation->run() == FALSE)
	        {
		        ...
		        if($_FILES['photo']['name']!='')
		        {
		            ...
		            $config['overwrite']=true;
		            $this->upload->initialize($config);
		            $this->check_file_type($imagepath,'jpg','candidate_images');
		        ...
		        if($_FILES['passport']['name']!='')
		        {
		            ...
		            $config['overwrite']=true;
		            $this->upload->initialize($config);
		            $this->check_file_type($pimagepath,'jpg','candidate_passports');
		        ...
		        if($_FILES['resume']['name']!='')
		        {
		            ...
		            $config['overwrite']=true;
		            $this->upload->initialize($config);
		            $this->check_file_type($rimagepath,'pdfdocdocx','candidate_resumes');
				...
				foreach($post['company_name'] as $key => $cmp){
					...
					$Arr_work = array('candidate_applied_id'=>$insert_id,'company_name'=>$cmp,'designation'=>$post['designation'][$key],'work_duration_start'=>$post['work_duration_start'][$key],'work_duration_end'=>$post['work_duration_end'][$key],'years'=>$years,'months'=>$months,'remote_in_career_page'=>@$post['remote_in_carrer'],'is_active'=>1,'created_at'=>$created_date);
					$this->db->insert('candidate_work_expericence',$Arr_work);
				}
				foreach($post['type'] as $key => $edu){
					if($post['edu_duration_start'][$key]!='' && $post['edu_duration_end'][$key]!=''){
						$EWorkStartDate=date('Y-m-01', strtotime($post['edu_duration_start'][@$key]));
						$EWorkEndDate=date('Y-m-30', strtotime($post['edu_duration_end'][@$key]));
						$Ediff = abs(strtotime($EWorkEndDate) - strtotime($EWorkStartDate));
						$Eyears = floor($Ediff / (365*60*60*24));
						$Emonths = floor(($Ediff - $Eyears * 365*60*60*24) / (30*60*60*24));
					}else{
						$Eyears = '';
						$Emonths = '';
					}
					$Arr_edu = array('candidate_applied_id'=>$insert_id,'type_id'=>$edu,'course_id'=>$post['course'][$key],'specialisation'=>$post['specialisation'][$key],'institution_name'=>$post['institution_name'][$key],'edu_duration_start'=>$post['edu_duration_start'][$key],'edu_duration_end'=>$post['edu_duration_end'][$key],'edu_years'=>$Eyears,'edu_months'=>$Emonths,'is_active'=>1,'created_at'=>$created_date);
	...
	public function candidate_profile($candidate_applied_id='')
	{
		...
		$GetCompanyJobIds=$this->db->get_where('candidate_applied_jobs', array('candidate_applied_id='=>$candidate_applied_id))->row_array();
		$data['company_id']=$GetCompanyJobIds['company_id'];
		$data['job_id']=$GetCompanyJobIds['job_id'];
		$data['statusVal']=$GetCompanyJobIds['status'];
		$data['comments']=$this->db->query("SELECT `candidate_job_status_logs_id`, `emp_id`, `candidate_applied_id`, `company_id`, `job_id`, `status`, `date`, `comments` FROM `candidate_job_status_logs` WHERE `candidate_applied_id`=$candidate_applied_id ORDER BY `candidate_job_status_logs_id` DESC")->result_array();
		$data['status']=$this->load->view("admin/get_job_status",$data,true);
	...
	public function change_candidate_status()
	{
		...
		$data['statusVal']=$value;
		$data['candidate_applied_id']=$candidate_applied_id;
		echo $this->load->view("admin/get_job_status",$data,true);
	}
		...
		$config = array( 
			...
			if ($this->form_validation->run() == FALSE)
	        {
				...
				foreach($post['job_work_expericence_id'] as $key => $work){
					if($post['work_duration_start'][$key]!='' && $post['work_duration_end'][$key]!=''){
						$WorkStartDate=date('Y-m-01', strtotime($post['work_duration_start'][@$key]));
						$WorkEndDate=date('Y-m-30', strtotime($post['work_duration_end'][@$key]));
						$diff = abs(strtotime($WorkEndDate) - strtotime($WorkStartDate));
						$years = floor($diff / (365*60*60*24));
					...
					$Arr_work = array('company_name'=>$post['company_name'][$key],'designation'=>$post['designation'][$key],'work_duration_start'=>$post['work_duration_start'][$key],'work_duration_end'=>$post['work_duration_end'][$key],'years'=>$years,'months'=>$months,'remote_in_career_page'=>@$post['remote_in_carrer'],'updated_at'=>$created_date);
				...
				foreach($post['candidate_education_id'] as $key => $edu){
					...
					$Arr_edu = array('type_id'=>$post['type'][$key],'course_id'=>$post['course'][$key],'specialisation'=>$post['specialisation'][$key],'institution_name'=>$post['institution_name'][$key],'edu_duration_start'=>$post['edu_duration_start'][$key],'edu_duration_end'=>$post['edu_duration_end'][$key],'edu_years'=>$Eyears,'edu_months'=>$Emonths,'updated_at'=>$created_date);
	...
	public function candidate_attachmeents()
	{
        ...
        else
        {
		        ...
		        foreach ($_FILES['file']['name'] as $key => $image)
		        {
			            ...
			            $this->load->library('upload');
			            $this->upload->initialize($config);
			            $this->check_file_type($filePath,'pdf','candidate_attachmeents');
			            if(!$this->upload->do_upload('images'))
			            {
			                $this->session->set_flashdata('failed','<div class="alert alert-danger msgfade"><strong>Please upload jpg,jpeg,png,pdf formates only</strong></div>');
			                redirect($_SERVER['HTTP_REFERER']);
	...
	public function check_file_type($filepath='',$type='',$assetpath='')
	{
		$admin_id=$this->session->userdata('emp_id');
		if($filepath!='' && $type!='' && $assetpath!='')
		{
			$file_ext = strtolower(pathinfo($filepath,PATHINFO_EXTENSION));
			@unlink("assets/".$assetpath."/".$filepath."/".$file_ext);
			if($type=='pdf')
			{
			...
			}else if($type=='onlypdf'){
				if($file_ext=='pdf')
		        {

		        }else{
		        	$this->session->set_flashdata('failed','<div class="alert alert-danger msgfade"><strong>Please upload pdf formates only</strong></div>');
		        	redirect($_SERVER['HTTP_REFERER']);
		        }
			}else if($type=='pdfdocdocx'){
				...
				$this->session->set_flashdata('failed','<div class="alert alert-danger msgfade"><strong>Opps! something went to wrong...</strong></div>');
				redirect($_SERVER['HTTP_REFERER']);
			}
		}else{
			$this->session->set_flashdata('failed','<div class="alert alert-danger msgfade"><strong>Opps! something went to wrong...</strong></div>');
	...
	{
		checklogin_admin('Claims');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Claims');
		$data['active_menu']='claims';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/claims');
		$this->load->view('admin/footer');
	}
	public function get_claims_list()
	{
		...
		$records=array();
		$returndata=array();
		$totalcount=$this->loginmodel->search_claim_Details(1,$start,$length,$search['value'],$column,$dir);
		$returndata['recordsTotal']=0;
		$returndata['recordsFiltered']=0;
		$returndata['recordsTotal']+=$totalcount;
		if($search['value']!=''){
		...
		$stu['output'] = $this->loginmodel->search_claim_Details(2,$start,$length,$search['value'],$column,$dir);
		$stu['returndata']=$returndata;
		if(is_array($stu['output']) && count($stu['output'])>0){
				foreach($stu['output'] as $k=>$res){
					$NewArr=array();
					$NewArr[]=$k+1;
					if($res['claim_type_id']!='')
					{
						$claim_type_id=$res['claim_type_id'];
						$ClaimType=$this->db->query("SELECT `claim_type_id`, `name`, `is_active`, `created_at` FROM `claim_type` WHERE `claim_type_id`=$claim_type_id")->row_array();
					}else{
						$ClaimType['name']='';
					}
					$NewArr[]=$res['fname'].' '.$res['lname'].' ('.$res['emp_code'].')';
					$NewArr[]=@$ClaimType['name'];
					$NewArr[]=DD_M_YY($res['date']);
					$NewArr[]='ZAR '.$res['amount'];
					$NewArr[]=$res['comments'];
					if($res['is_approved']=='Pending')
					{
						$NewArr[]='<span style="color:#0069d9">Pending</span>';
					}else if($res['is_approved']=='Approved')
					{
						$NewArr[]='<span style="color:#28a745">Approved</span><br>Approved Date: '.date("d-M-Y H:i:s", strtotime($res['accepted_rejected_dt']));
					}else{
						$NewArr[]='<span style="color:#FB7474">Rejected</span><br>Rejected Date: '.date("d-M-Y H:i:s", strtotime($res['accepted_rejected_dt']));
					}
					if($res['claim_id']!='')
					{
						$claim_id=$res['claim_id'];
						$GetImgs=$this->db->query("SELECT `claim_document_id`, `claim_id`, `file_name`, `file_name_path`, `is_active`, `created_at` FROM `claim_documents` WHERE `claim_id`=$claim_id")->result_array();
						$ArrImgs=array();
						$a=1;
						foreach ($GetImgs as $key => $Img)
						{
							$ArrImgs[]=$a.') <a href="'.base_url().'assets/claim_documents/'.$Img['file_name_path'].'" target="_blank">'.$Img['file_name'].'</a><br>';
							$a++;
						}
					}else{
						$ArrImgs[]=array();
					...
					$NewArr[]=$ArrImgs;
					$NewArr[]=$res['accepted_rejected_comments'];
					if($res['is_approved']=='Pending')
					{
					    $NewArr[]='<button class="btn btn-info waves-effect waves-light" onclick="approved('.$res['claim_id'].');"> Approve </button> | <button class="btn btn-danger waves-effect waves-light" onclick="rejected('.$res['claim_id'].');"> Reject </button><br>';
					}else{
					   if($res['is_approved']=='Approved')
					    {
						    $NewArr[]='<span style="color:#28a745">Approved</span>';
					    }else{
						    $NewArr[]='<span style="color:#FB7474">Rejected</span>';
					    }
					}
					$returndata['data'][]=$NewArr;
				}
	...
	public function approved_rejected_claims()
	{
		checklogin_admin('Claims');
		$admin_id=$this->session->userdata('emp_id');
		$claim_id=$this->input->post('claims_model_id');
		$sta=$this->input->post('approved_rejected_sta');
		if($claim_id=='' || $sta==''){
		    $this->session->set_flashdata('failed','Opps! This Claims Status Changed Failed...');
		...
		if($sta==1){
			...
			$post['is_approved']='Rejected';
		}
		$post['accepted_rejected_comments']=$this->input->post('accepted_rejected_comments');
		$post['accepted_rejected_dt']=date("Y-m-d H:i:s");
		$this->db->where('claim_id',$claim_id);
		$res=$this->db->update('claims',$post);
		if($res==1){
			$this->session->set_flashdata('success','Claim Status Changed Successfully...');
		}else{
			$post['is_delete']='No';
			$this->db->where('payruns_id',$payruns_id);
			$this->db->update('payrun',$post);
			$this->session->set_flashdata('failed','This Claims Status Changed Failed...');
		}
		$this->admin_logs('NULL',$claim_id,'Status Change Claim');
		redirect('admin/claims');
	}
	public function employee_compensation($status='')
	{
		checklogin_admin('Employee Compensation');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Employee Compensation');
		$data['status']=$status;
		$data['active_menu']='claims';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/employee_compensation');
		$this->load->view('admin/footer');
	}
	public function get_employee_compensation_list()
	{
		...
		$records=array();
		$returndata=array();
		$totalcount=$this->loginmodel->search_employee_compensation_Details(1,$start,$length,$search['value'],$column,$dir,$status);
		$returndata['recordsTotal']=0;
		$returndata['recordsFiltered']=0;
		$returndata['recordsTotal']+=$totalcount;
		if($search['value']!=''){
		...
		$stu['output'] = $this->loginmodel->search_employee_compensation_Details(2,$start,$length,$search['value'],$column,$dir,$status);
		$stu['returndata']=$returndata;
		if(is_array($stu['output']) && count($stu['output'])>0){
				foreach($stu['output'] as $k=>$res){
					$NewArr=array();
					$NewArr[]=$k+1;
					$NewArr[]=$res['fname'].' '.$res['lname'].' ('.$res['emp_code'].')';
				    $NewArr[]='ZAR '.number_format((float)$res['per_hour_rate'], 2, '.', '');
					$NewArr[]='ZAR '.number_format((float)$res['monthly_salary'], 2, '.', '');
					$NewArr[]='ZAR '.number_format((float)$res['ctc'], 2, '.', '');
					$NewArr[]=$res['contact_start_dt'];
					$NewArr[]=$res['contact_end_dt'];
					$NewArr[]=$res['employment_type'];
					$NewArr[]='<a href="'.base_url().'admin/edit_employee_compensation/'.$res['employee_compensation_id'].'" class="btn btn-info waves-effect waves-light"><i class="fa fa-edit" style="color: #fff !important;"></i> Edit </a>';
					$returndata['data'][]=$NewArr;
				}
		}else{
			$returndata['data']=array();
	...
	public function add_employee_compensation()
	{
		checklogin_admin('Employee Compensation');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Employee Compensation');
		$data['active_menu']='employee_compensation';
		$data['employees']=$this->db->query("SELECT `emp_id`, `fname`, `lname`, `emp_code` FROM `employees` WHERE `is_active`=1")->result_array();
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/add_employee_compensation');
		$this->load->view('admin/footer');
	}
	public function get_employment_type()
	{
		...
		$data['GetRolesAccess']=$this->read_write('Employee Compensation');
		$emp_id=$this->input->post('emp_id');
		$employee_type=array('Independent Contractor','Fixed term Employee','Permanent Employee');
		$selected_employee_type=$this->db->query("SELECT `emp_id`, `fname`, `lname`, `emp_code`, `employment_type` FROM `employees` WHERE `emp_id`=$emp_id")->row_array();
		echo json_encode(array('Val'=>$employee_type,'SelectedVal'=>$selected_employee_type['employment_type']));
	}
	public function save_employee_compensation()
	{
		checklogin_admin('Employee Compensation');
		$admin_id=$this->session->userdata('emp_id');
		$post=$this->input->post();
		...
		$config = array( 
		array( 'field'=> 'emp_id', 'label'=> 'Employee','rules'=> 'required'),
		array( 'field'=> 'per_hour_rate', 'label'=> 'Per Hrs Rate','rules'=> 'required'),
		array( 'field'=> 'monthly_salary', 'label'=> 'Monthly Salary','rules'=> 'required'),
		array( 'field'=> 'ctc', 'label'=> 'CTC','rules'=> 'required'),
		array( 'field'=> 'contact_start_dt', 'label'=> 'Contact Start Date','rules'=> 'required'),
		array( 'field'=> 'contact_end_dt', 'label'=> 'Contact End Date','rules'=> 'required'),
		);
		$this->form_validation->set_rules($config);
		if ($this->form_validation->run() == FALSE)
        {
            $this->session->set_flashdata('msg','<div class="alert alert-danger msgfade"><strong>'.validation_errors().'</strong></div>');
            redirect('admin/employee_compensation');
        }else{
        	$per_hour_rate=$post['per_hour_rate'];
        	$monthly_salary=$post['monthly_salary'];
        	$PerHourRate=$monthly_salary/168;
        	$post['per_hour_rate']=$PerHourRate;
        	$post['monthly_salary']=$PerHourRate*168;
        	$post['contact_start_dt']=YY_MM_DD($post['contact_start_dt']);
        	$post['contact_end_dt']=YY_MM_DD($post['contact_end_dt']);
        	$post['is_active']=1;
			$post['created_at']=$created_date;
			$emp_id=$post['emp_id'];
        	$ContactStartDt=$post['contact_start_dt'];
        	$ContactEndDt=$post['contact_end_dt'];
			if($post['employee_compensation_id']=='')
			{
	        	$ChkStartDate=$this->db->query("SELECT `employee_compensation_id`, `emp_id`, `per_hour_rate`, `monthly_salary`, `ctc`, `contact_start_dt`, `contact_end_dt` FROM `employee_compensation` WHERE `contact_start_dt` <='$ContactStartDt' AND `contact_end_dt`>='$ContactEndDt' AND `emp_id` = $emp_id")->num_rows();
	        	$ChkEndDate=$this->db->query("SELECT `employee_compensation_id`, `emp_id`, `per_hour_rate`, `monthly_salary`, `ctc`, `contact_start_dt`, `contact_end_dt` FROM `employee_compensation` WHERE `contact_end_dt`>='$ContactStartDt' AND `contact_start_dt` <='$ContactEndDt' AND `emp_id` = $emp_id")->num_rows();
				if($ChkStartDate==0 && $ChkEndDate==0)
        		{
					$res=$this->db->insert('employee_compensation',$post);
					$insert_id = $this->db->insert_id();
					$ArrEmp=array('employment_type'=>$post['employment_type']);
					$this->db->where('emp_id',$post['emp_id']);
					$this->db->update('employees',$ArrEmp);
					$this->admin_logs('NULL',$insert_id,'Save Employee Compensation');
					if($res==1){
						$this->session->set_flashdata('success','<div class="alert alert-success msgfade"><strong>Employee Compensation Saved Successfully...</strong></div>');
					}else{
						$this->session->set_flashdata('failed','<div class="alert alert-warning msgfade"><strong>Employee Compensation Saved failed!...</strong></div>');
					}
				}else{
					$this->session->set_flashdata('failed','<div class="alert alert-warning msgfade"><strong>This Employee Compensation Already Existed!...</strong></div>');
					redirect('admin/employee_compensation/');
				}
			}else{
				$employee_compensation_id=$post['employee_compensation_id'];
				$UpdateChkStartDate=$this->db->query("SELECT `employee_compensation_id`, `emp_id`, `per_hour_rate`, `monthly_salary`, `ctc`, `contact_start_dt`, `contact_end_dt` FROM `employee_compensation` WHERE `contact_start_dt`<='$ContactStartDt' AND `contact_end_dt`>='$ContactEndDt' AND `emp_id`=$emp_id AND `employee_compensation_id`!=$employee_compensation_id")->num_rows();
	        	$UpdateChkEndDate=$this->db->query("SELECT `employee_compensation_id`, `emp_id`, `per_hour_rate`, `monthly_salary`, `ctc`, `contact_start_dt`, `contact_end_dt` FROM `employee_compensation` WHERE `contact_end_dt`>='$ContactStartDt' AND `contact_start_dt` <='$ContactEndDt' AND `emp_id`=$emp_id AND `employee_compensation_id`!=$employee_compensation_id")->num_rows();
				if($UpdateChkStartDate==0 && $UpdateChkEndDate==0)
        		{
					$per_hour_rate=$post['per_hour_rate'];
		        	$monthly_salary=$post['monthly_salary'];
		        	$PerHourRate=$monthly_salary/168;
		        	...
		        	$post['monthly_salary']=$PerHourRate*168;
					$post['is_active']=1;
					$post['updated_at']=$created_date;
					$this->db->where('employee_compensation_id',$post['employee_compensation_id']);
					$res=$this->db->update('employee_compensation',$post);
					$ArrEmp=array('employment_type'=>$post['employment_type']);
					$this->db->where('emp_id',$post['emp_id']);
					$this->db->update('employees',$ArrEmp);
					$this->admin_logs('NULL',$post['employee_compensation_id'],'Update Employee Compensation');
					if($res==1){
						$this->session->set_flashdata('success','<div class="alert alert-success msgfade"><strong>Employee Compensation Updated Successfully...</strong></div>');
					}else{
						$this->session->set_flashdata('failed','<div class="alert alert-warning msgfade"><strong>Employee Compensation Updated failed!...</strong></div>');
					}
				}else{
					$this->session->set_flashdata('failed','<div class="alert alert-warning msgfade"><strong>This Employee Compensation Already Existed!...</strong></div>');
					redirect('admin/employee_compensation/');
	...
	public function edit_employee_compensation($employee_compensation_id='')
	{
		checklogin_admin('Employee Compensation');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Employee Compensation');
		$data['employee_compensation']=$this->db->query("SELECT `employee_compensation_id`, `emp_id`, `per_hour_rate`, `monthly_salary`, `ctc`, `contact_start_dt`, `contact_end_dt`, `employment_type`, `is_active`, `created_at` FROM `employee_compensation` WHERE `employee_compensation_id`=$employee_compensation_id")->row_array();
		$data['active_menu']='employee_compensation';
		$data['employees']=$this->db->query("SELECT `emp_id`, `fname`, `lname`, `emp_code` FROM `employees` WHERE `is_active`=1")->result_array();
		$data['employment_type']=array('Independent Contractor','Fixed term Employee','Permanent Employee');
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/add_employee_compensation');
		$this->load->view('admin/footer');
	}
	public function is_cron_contract_list()
	{
		$Date=date('Y-m-d');
		$Plus3MonthsDate=date('Y-m-d', strtotime("+3 months"));
		$GetAllContractLists=$this->db->query("SELECT `employee_compensation_id`, `emp_id`, `per_hour_rate`, `monthly_salary`, `ctc`, `contact_start_dt`, `contact_end_dt`, `is_active`, `created_at` FROM `employee_compensation` WHERE `contact_start_dt`>='$Date' AND `contact_end_dt`<='$Plus3MonthsDate' AND `is_active`=1")->result_array();
		// echo $this->db->last_query();exit;
		if(!empty($GetAllContractLists))
		{
			foreach ($GetAllContractLists as $res)
			{
				$emp_id = $res['emp_id'];
				$Get_managers_ids=$this->db->query("SELECT `emp_id`, `fname`, `lname`, `emp_code`, `email_id`, `reporting_manager_id`, `hr_manager_id`, `lead_manager_id` FROM `employees` WHERE `emp_id`=$emp_id")->row_array();
				$hr_manager_id=$Get_managers_ids['hr_manager_id'];
				$lead_manager_id=$Get_managers_ids['lead_manager_id'];
				$reporting_manager_id=$Get_managers_ids['reporting_manager_id'];
				$data['GetEmpDetails']=$this->db->query("SELECT `emp_id`, `fname`, `lname`, `email_id`, `emp_code`, `lead_manager_id` FROM `employees` WHERE `emp_id`=$emp_id")->row_array();
				$to_email=$data['GetEmpDetails']['email_id'];
				$data['contact_start_dt'] = $inprogress_leave['contact_start_dt'];
				$data['contact_end_dt'] = $inprogress_leave['contact_end_dt'];
				$this->sent_email_to_contract_list('Founder','kishor@vibhotech.com',$data);
				$this->sent_email_to_contract_list('HR',$hr_manager_id,$data);
				$this->sent_email_to_contract_list('Reporting Manager',$reporting_manager_id,$data);
			}
		}else{
			echo "no data found...";
		}
	}
	public function sent_email_to_contract_list($Type='',$email_id='',$data='')
	{
		if($Type!='' && $email_id!='' && !empty($data))
		{	
			$data['Type']=$Type;
			$mesg = $this->load->view('admin/is_cron_contract_list',$data,true);
			$config=email_helper();
			$this->load->library('email');
			$this->email->initialize($config);
			$this->email->from('no-reply@vibhotech.com', 'Vibho Employee Solutions');
			$this->email->to($email_id); 
			$this->email->subject('Contract List');
			$this->email->message($mesg);
			$this->email->set_mailtype("html");
			$this->email->send();
		}
	}
	public function export_employee_compensation()
	{
		checklogin_admin('Employee Compensation');
		$admin_id=$this->session->userdata('emp_id');
		$dt=$this->db->query("SELECT `employee_compensation_id`, `emp_id`, `per_hour_rate`, `monthly_salary`, `ctc`, `contact_start_dt`, `contact_end_dt`, `employment_type`, `is_active`, `created_at`, `updated_at` FROM `employee_compensation`")->result_array();
		$date =date('d-M-Y');
		$name = 'Employee_Compensations_'.$date;
        $name=$name.'.xls';
        header("Content-Type: application/vnd.ms-excel");
		header("Content-disposition: Attachment; filename=$name");
		$aa ="S.No\tEmployee Name\tPer Hr Rate\tMonthly Salary\tCTC\tContract Start Date\tContract End Date\tEmployement Type\tCreated Date\n";
		$counter=1;
		foreach($dt as $k=>$val){
			if($val['emp_id']!=''){
				$EID=$val['emp_id'];
				$GetEmpDetails=$this->db->query("SELECT `emp_id`, `fname`, `lname`, `email_id`, `emp_code` FROM `employees` WHERE `emp_id`=$EID")->row_array();
				$empName= $GetEmpDetails['fname'].' '.$GetEmpDetails['lname'].' '.$GetEmpDetails['emp_code'];
			}else{
				$empName='';
			}
			$aa.=$counter."\t".$empName."\t".'ZAR '.number_format((float)$val['per_hour_rate'], 2, '.', '')."\t".'ZAR '.number_format((float)$val['monthly_salary'], 2, '.', '')."\t".'ZAR '.number_format((float)$val['ctc'], 2, '.', '')."\t".DD_M_YY($val['contact_start_dt'])."\t".DD_M_YY($val['contact_end_dt'])."\t".$val['employment_type']."\t".DD_M_YY_H_I_Sa($val['created_at'])."\n";
             $counter++;
        }
        echo $aa;
	}
	public function rate_calculator_config()
	{
		checklogin_admin('Rate Calculator Config');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Rate Calculator Config');
		$data['active_menu']='rate_calculator_config';
		$data['rate_calculator_config']=$this->db->query("SELECT `rate_calculator_config_id`, `min_client_rate`, `max_client_rate`, `is_active`, `created_at`, `updated_at` FROM `rate_calculator_config`")->result_array();
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/rate_calculator_config');
		$this->load->view('admin/footer');
	}
	public function add_rate_calculator_config()
	{
		checklogin_admin('Rate Calculator Config');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Rate Calculator Config');
		$data['active_menu']='rate_calculator_config';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/add_rate_calculator_config');
		$this->load->view('admin/footer');
	}
	public function save_rate_calculator_config()
	{
		checklogin_admin('Rate Calculator Config');
		$admin_id=$this->session->userdata('emp_id');
		$post=$this->input->post();
		$created_date =date("Y-m-d H:i:s");
		$config = array( 
		array( 'field'=> 'min_client_rate', 'label'=> 'Min Client Rate','rules'=> 'required'),
		array( 'field'=> 'max_client_rate', 'label'=> 'Max Client Rate','rules'=> 'required'),
		);
		$this->form_validation->set_rules($config);
		if ($this->form_validation->run() == FALSE)
        {
            $this->session->set_flashdata('msg','<div class="alert alert-danger msgfade"><strong>'.validation_errors().'</strong></div>');
            redirect('admin/rate_calculator_config');
        }else{
        	$post['is_active']=1;
			$post['updated_at']=$created_date;
			if($post['rate_calculator_config_id']=='')
			{
				$this->db->insert('rate_calculator_config',$post);
				$this->admin_logs('NULL',$insert_id,'Save abc');
				if($res==1){
					$this->session->set_flashdata('success','<div class="alert alert-success msgfade"><strong>Notifications Saved Successfully...</strong></div>');
				}else{
					$this->session->set_flashdata('failed','<div class="alert alert-warning msgfade"><strong>Notifications Saved failed!...</strong></div>');
				...
				$this->db->where('rate_calculator_config_id',$post['rate_calculator_config_id']);
				$res=$this->db->update('rate_calculator_config',$post);
				// echo $this->db->last_query();exit;
				$this->admin_logs('NULL',$insert_id,'Save abc');
				if($res==1){
					$this->session->set_flashdata('success','<div class="alert alert-success msgfade"><strong>Rate Calculator Config Updated Successfully...</strong></div>');
				}else{
					$this->session->set_flashdata('failed','<div class="alert alert-warning msgfade"><strong>Rate Calculator Config Updated failed!...</strong></div>');
				}
			}
			redirect('admin/rate_calculator_config');
		}
	}
	public function edit_rate_calculator_config($rate_calculator_config_id='')
	{
		checklogin_admin('Rate Calculator Config');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Rate Calculator Config');
		$data['rate_calculator_config']=$this->db->query("SELECT `rate_calculator_config_id`, `min_client_rate`, `max_client_rate`, `is_active`, `created_at`, `updated_at` FROM `rate_calculator_config` WHERE `rate_calculator_config_id`=$rate_calculator_config_id")->row_array();
		$data['active_menu']='rate_calculator_config';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/add_rate_calculator_config');
		$this->load->view('admin/footer');
	}
	public function change_rate_calculator_config_status($rate_calculator_config_id='',$sta='')
	{
	    checklogin_admin('Rate Calculator Config');
	    $data['GetRolesAccess']=$this->read_write('Rate Calculator Config');
	    $this->admin_logs('NULL',$client_id,'Change Rate Calculator Config',$sta);
		$data = array('is_active'=>$sta);
		$this->db->where('rate_calculator_config_id',$rate_calculator_config_id);
		$res=$this->db->update('rate_calculator_config',$data);
		if ($res==1)
		{
			$this->session->set_flashdata('success','<div class="alert alert-success msgfade"><strong>Rate Calculator Config Status Successfully...</strong></div>');
		}
		else
		{
			$this->session->set_flashdata('failed','<div class="alert alert-warning msgfade"><strong>Rate Calculator Config Status failed!...</strong></div>');
		}
		redirect('admin/rate_calculator_config');
	}
	public function client_based_rate_config($status='')
	{
		checklogin_admin('Client Based Rate Config');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Client Based Rate Config');
		$data['status']=$status;
		$data['active_menu']='claims';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/client_based_rate_config');
		$this->load->view('admin/footer');
	}
	public function get_client_based_rate_config_list()
	{
		$start=$this->input->get('start')?intval( $this->input->get('start') ) :0;
		$length=$this->input->get('length')?intval( $this->input->get('length') ) :0;
		$search=$this->input->get('search')?$this->input->get('search'):array('value'=>'');
		...
		$records=array();
		$returndata=array();
		$totalcount=$this->loginmodel->search_client_based_rate_config_Details(1,$start,$length,$search['value'],$column,$dir);
		$returndata['recordsTotal']=0;
		$returndata['recordsFiltered']=0;
		$returndata['recordsTotal']+=$totalcount;
		if($search['value']!=''){
		...
		$stu['output'] = $this->loginmodel->search_client_based_rate_config_Details(2,$start,$length,$search['value'],$column,$dir);
		$stu['returndata']=$returndata;
		if(is_array($stu['output']) && count($stu['output'])>0){
				foreach($stu['output'] as $k=>$res){
					$NewArr=array();
					$NewArr[]=$k+1;
					$NewArr[]=$res['name'];
					$NewArr[]='ZAR '.$res['client_rate_card'];
					$NewArr[]='ZAR '.$res['min_candidate_rate'];
					$NewArr[]='ZAR '.$res['max_candidate_rate'];
					$NewArr[]='<a href="'.base_url().'admin/edit_client_based_rate_config/'.$res['client_based_rate_config_id'].'" class="btn btn-info waves-effect waves-light"><i class="fa fa-edit" style="color: #fff !important;"></i> Edit </a>';
					$returndata['data'][]=$NewArr;
				}
		}else{
			$returndata['data']=array();
	...
	public function add_client_based_rate_config()
	{
		checklogin_admin('Client Based Rate Config');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Client Based Rate Config');
		$data['rate_calculator_config']=$this->db->query("SELECT `rate_calculator_config_id`, `min_client_rate`, `max_client_rate`, `is_active`, `created_at`, `updated_at` FROM `rate_calculator_config`")->row_array();
		$data['active_menu']='client_based_rate_config';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/add_client_based_rate_config');
		$this->load->view('admin/footer');
	}
	public function save_client_based_rate_config()
	{
		checklogin_admin('Client Based Rate Config');
		$admin_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Client Based Rate Config');
		$rate_calculator_config=$this->db->query("SELECT `rate_calculator_config_id`, `min_client_rate`, `max_client_rate`, `is_active`, `created_at`, `updated_at` FROM `rate_calculator_config`")->row_array();
		$post=$this->input->post();
		$created_date =date("Y-m-d H:i:s");
		$config = array( 
		array( 'field'=> 'name', 'label'=> 'Employee Name','rules'=> 'required'),
		array( 'field'=> 'client_rate_card', 'label'=> 'Client Rate Card','rules'=> 'required'),
		array( 'field'=> 'min_candidate_rate', 'label'=> 'Mai Candidate Rate','rules'=> 'required'),
		array( 'field'=> 'max_candidate_rate', 'label'=> 'Max Candidate Rate','rules'=> 'required'),
		);
		$this->form_validation->set_rules($config);
		if ($this->form_validation->run() == FALSE)
        {
            $this->session->set_flashdata('msg','<div class="alert alert-danger msgfade"><strong>'.validation_errors().'</strong></div>');
            redirect('admin/client_based_rate_config');
        }else if(@$rate_calculator_config["min_client_rate"]=='' && @$rate_calculator_config["max_client_rate"]==''){
        	$this->session->set_flashdata('msg','<div class="alert alert-danger msgfade"><strong>Opps! rate calculator config is empty...</strong></div>');
            redirect('admin/client_based_rate_config');
        }else{
        	$MinClientRate =@$rate_calculator_config["min_client_rate"];
            $MaxClientRate =@$rate_calculator_config["max_client_rate"];
            $GetClientRateCard=$post['client_rate_card'];
            $MinCandidateRate=($GetClientRateCard)/($MaxClientRate);
            $FloatMin=number_format((float)$MinCandidateRate, 2, '.', '');
            $post['min_candidate_rate']=$FloatMin;
            $MaxCandidateRate=($GetClientRateCard)/($MinClientRate);
            $FloatMax=number_format((float)$MaxCandidateRate, 2, '.', '');
            $post['max_candidate_rate']=$FloatMax;
            $post['min_client_rate']=@$rate_calculator_config["min_client_rate"];
		    $post['max_client_rate']=@$rate_calculator_config["max_client_rate"];
        	$post['is_active']=1;
			$post['created_at']=$created_date;
			if($post['client_based_rate_config_id']=='')
			{
				$res=$this->db->insert('client_based_rate_config',$post);
				$insert_id = $this->db->insert_id();
				$this->admin_logs('NULL',$insert_id,'Save Client Based Rate Config');
				if($res==1){
					$this->session->set_flashdata('success','<div class="alert alert-success msgfade"><strong>Client Based Rate Config Saved Successfully...</strong></div>');
				}else{
					$this->session->set_flashdata('failed','<div class="alert alert-warning msgfade"><strong>Client Based Rate Config Saved failed!...</strong></div>');
				}
			}else{
				$post['is_active']=1;
				$post['updated_at']=$created_date;
				$this->db->where('client_based_rate_config_id',$post['client_based_rate_config_id']);
				$res=$this->db->update('client_based_rate_config',$post);
				$this->admin_logs('NULL',$post['client_based_rate_config_id'],'Update Client Based Rate Config');
				if($res==1){
					$this->session->set_flashdata('success','<div class="alert alert-success msgfade"><strong>Client Based Rate Config Updated Successfully...</strong></div>');
				}else{
					$this->session->set_flashdata('failed','<div class="alert alert-warning msgfade"><strong>Client Based Rate Config Updated failed!...</strong></div>');
				}
			}
			redirect('admin/client_based_rate_config/');
		}
	}
	public function edit_client_based_rate_config($client_based_rate_config_id='')
	{
		checklogin_admin('Client Based Rate Config');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Client Based Rate Config');
		$data['client_based_rate_config']=$this->db->query("SELECT `client_based_rate_config_id`, `name`, `client_rate_card`, `min_candidate_rate`, `max_candidate_rate`, `is_active`, `created_at`, `updated_at` FROM `client_based_rate_config` WHERE `client_based_rate_config_id`=$client_based_rate_config_id")->row_array();
		$data['rate_calculator_config']=$this->db->query("SELECT `rate_calculator_config_id`, `min_client_rate`, `max_client_rate`, `is_active`, `created_at`, `updated_at` FROM `rate_calculator_config`")->row_array();
		$data['active_menu']='client_based_rate_config';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/add_client_based_rate_config');
	...
	public function candidate_based_rate_config($status='')
	{
		checklogin_admin('Candidate Based Rate Config');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Candidate Based Rate Config');
		$data['status']=$status;
		$data['active_menu']='candidate_based_rate_config';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/candidate_based_rate_config');
		$this->load->view('admin/footer');
	}
	public function get_candidate_based_rate_config_list()
	{
		$start=$this->input->get('start')?intval( $this->input->get('start') ) :0;
		$length=$this->input->get('length')?intval( $this->input->get('length') ) :0;
		$search=$this->input->get('search')?$this->input->get('search'):array('value'=>'');
		...
		$records=array();
		$returndata=array();
		$totalcount=$this->loginmodel->search_candidate_based_rate_config_Details(1,$start,$length,$search['value'],$column,$dir);
		$returndata['recordsTotal']=0;
		$returndata['recordsFiltered']=0;
		$returndata['recordsTotal']+=$totalcount;
		if($search['value']!=''){
		...
		$stu['output'] = $this->loginmodel->search_candidate_based_rate_config_Details(2,$start,$length,$search['value'],$column,$dir);
		$stu['returndata']=$returndata;
		if(is_array($stu['output']) && count($stu['output'])>0){
				foreach($stu['output'] as $k=>$res){
					$NewArr=array();
					...
					$NewArr[]=$res['name'];
					$NewArr[]=$res['id_no'];
					$NewArr[]='ZAR '.$res['excepted_rate_in_hrs'];
					$NewArr[]='ZAR '.$res['min_client_rate'];
					$NewArr[]='ZAR '.$res['max_client_rate'];
					$NewArr[]='<a href="'.base_url().'admin/edit_candidate_based_rate_config/'.$res['candidate_based_rate_config_id'].'" class="btn btn-info waves-effect waves-light"><i class="fa fa-edit" style="color: #fff !important;"></i> Edit </a>';
					$returndata['data'][]=$NewArr;
				}
		}else{
			$returndata['data']=array();
	...
	public function add_candidate_based_rate_config()
	{
		checklogin_admin('Candidate Based Rate Config');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Candidate Based Rate Config');
		...
		$data['active_menu']='candidate_based_rate_config';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/add_candidate_based_rate_config');
		$this->load->view('admin/footer');
	}
	public function save_candidate_based_rate_config()
	{
		checklogin_admin('Candidate Based Rate Config');
		$admin_id=$this->session->userdata('emp_id');
		$rate_calculator_config=$this->db->query("SELECT `rate_calculator_config_id`, `min_client_rate`, `max_client_rate`, `is_active`, `created_at`, `updated_at` FROM `rate_calculator_config`")->row_array();
		...
		$config = array( 
		array( 'field'=> 'name', 'label'=> 'Employee Name','rules'=> 'required'),
		array( 'field'=> 'id_no', 'label'=> 'ID No','rules'=> 'required'),
		array( 'field'=> 'excepted_rate_in_hrs', 'label'=> 'Candidate Expected Rate in Hrs','rules'=> 'required'),
		array( 'field'=> 'min_client_rate', 'label'=> 'Mai Client Rate','rules'=> 'required'),
		array( 'field'=> 'max_client_rate', 'label'=> 'Max Client Rate','rules'=> 'required'),
		);
		$this->form_validation->set_rules($config);
		if ($this->form_validation->run() == FALSE)
        {
            $this->session->set_flashdata('msg','<div class="alert alert-danger msgfade"><strong>'.validation_errors().'</strong></div>');
            redirect('admin/candidate_based_rate_config');
        }else if(@$rate_calculator_config["min_client_rate"]=='' && @$rate_calculator_config["max_client_rate"]==''){
        	$this->session->set_flashdata('msg','<div class="alert alert-danger msgfade"><strong>Opps! rate calculator config is empty...</strong></div>');
            redirect('admin/candidate_based_rate_config');
        }else{
            $MinClientRate =@$rate_calculator_config["min_client_rate"];
            $MaxClientRate =@$rate_calculator_config["max_client_rate"];
            $GetExceptedRateInHrs=$post['excepted_rate_in_hrs'];
            $MinClientRate=($GetExceptedRateInHrs)/($MinClientRate);
            $FloatMin=number_format((float)$MinClientRate, 2, '.', '');
            $post['min_client_rate']=$FloatMin;
            $MaxClientRate=($GetExceptedRateInHrs)/($MaxClientRate);
            $FloatMax=number_format((float)$MaxClientRate, 2, '.', '');
            $post['max_client_rate']=$FloatMax;
            $post['min_client_rate_config']=@$rate_calculator_config["min_client_rate"];
		        $post['max_client_rate_config']=@$rate_calculator_config["max_client_rate"];
        	$post['is_active']=1;
			$post['created_at']=$created_date;
			if($post['candidate_based_rate_config_id']=='')
			{
				$this->db->insert('candidate_based_rate_config',$post);
				$insert_id = $this->db->insert_id();
				$this->admin_logs('NULL',$insert_id,'Save Candidate Based Rate Config');
				if($res==1){
					$this->session->set_flashdata('success','<div class="alert alert-success msgfade"><strong>Candidate Based Rate Config Saved Successfully...</strong></div>');
				}else{
					$this->session->set_flashdata('failed','<div class="alert alert-warning msgfade"><strong>Candidate Based Rate Config Saved failed!...</strong></div>');
				}
			}else{
				$post['is_active']=1;
				$post['updated_at']=$created_date;
				$this->db->where('candidate_based_rate_config_id',$post['candidate_based_rate_config_id']);
				$res=$this->db->update('candidate_based_rate_config',$post);
				$this->admin_logs('NULL',$post['candidate_based_rate_config_id'],'Update Candidate Based Rate Config');
				if($res==1){
					$this->session->set_flashdata('success','<div class="alert alert-success msgfade"><strong>Candidate Based Rate Config Updated Successfully...</strong></div>');
				}else{
					$this->session->set_flashdata('failed','<div class="alert alert-warning msgfade"><strong>Candidate Based Rate Config Updated failed!...</strong></div>');
				}
			}
			redirect('admin/candidate_based_rate_config/');
		}
	}
	public function edit_candidate_based_rate_config($candidate_based_rate_config_id='')
	{
		checklogin_admin('Candidate Based Rate Config');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Candidate Based Rate Config');
		$data['contract_list']=$this->db->query("SELECT `candidate_based_rate_config_id`, `name`, `id_no`, `excepted_rate_in_hrs`, `min_client_rate`, `max_client_rate`, `is_active`, `created_at`, `updated_at` FROM `candidate_based_rate_config` WHERE `candidate_based_rate_config_id`=$candidate_based_rate_config_id")->row_array();
		$data['rate_calculator_config']=$this->db->query("SELECT `rate_calculator_config_id`, `min_client_rate`, `max_client_rate`, `is_active`, `created_at`, `updated_at` FROM `rate_calculator_config`")->row_array();
		$data['active_menu']='candidate_based_rate_config';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/add_candidate_based_rate_config');
	...
	{
		checklogin_admin('Nature Of Business');
		$admin_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Designations');
		$data['nature_of_business']=$this->db->query("SELECT `nature_of_business_id`, `name`, `is_active`, `created_at`, `updated_at` FROM `nature_of_business`")->result_array();
		$data['active_menu']='nature_of_business';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/nature_of_business');
		$this->load->view('admin/footer');
	}
	public function check_nature_of_business_existed()
	{
		checklogin_admin('Nature Of Business');
		$admin_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Nature Of Business');
		$business_name=trim($this->input->post('business_name'));
		$check=$this->db->get_where('nature_of_business',array('name'=>$business_name))->num_rows();
		if($check==0){
			$data=array('name'=>$business_name,'is_active'=>1,'created_at'=>date("Y-m-d H:i:s"));
			$res=$this->db->insert('nature_of_business',$data);
			$dataArr['get_nature_of_business']=$this->db->query("SELECT `nature_of_business_id`, `name`, `is_active`, `created_at`, `updated_at` FROM `nature_of_business` WHERE `is_active`=1")->result_array();
			$dataArr['get_nature_of_business']['res']=1;
			echo json_encode($dataArr);
		}else{
			$dataArr['get_nature_of_business']['res']=0;
			echo json_encode($dataArr);
		}
	}
	public function change_nature_of_business_status($nature_of_business_id='',$sta='')
	{
		checklogin_admin('Nature Of Business','Write');
	    $this->admin_logs('NULL',$nature_of_business_id,'Change Nature Of Business Status',$sta);
		$data = array('is_active'=>$sta);
		$this->db->where('nature_of_business_id',$nature_of_business_id);
		$res=$this->db->update('nature_of_business',$data);
		if ($res==1)
		{
			$this->session->set_flashdata('success','<div class="alert alert-success msgfade"><strong>Nature of Business Status Updated Successfully...</strong></div>');
		}
		else
		{
			$this->session->set_flashdata('failed','<div class="alert alert-warning msgfade"><strong>Nature of Business Status Update Failed!...</strong></div>');
		}
		redirect('admin/nature_of_business');
	}
	public function add_nature_of_business($nature_of_business_id='')
	{
		checklogin_admin('Nature Of Business','Write');
		$admin_id=$this->session->userdata('emp_id');
		if($nature_of_business_id!=''){
			$data['nature_of_business']=$this->db->query("SELECT `nature_of_business_id`, `name`, `is_active`, `created_at`, `updated_at` FROM `nature_of_business` WHERE `nature_of_business_id`=$nature_of_business_id")->row_array();
		}
		$data['active_menu']='nature_of_business';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/add_nature_of_business');
		$this->load->view('admin/footer');
	}
	public function save_nature_of_business()
	{
		checklogin_admin('Nature Of Business');
		$admin_id=$this->session->userdata('emp_id');
		$nature_of_business_id=$this->input->post('nature_of_business_id');
		$name=trim($this->input->post('name'));
		$created_at =date("Y-m-d H:i:s");
		$config = array( 
			array( 'field' => 'name', 'label' => 'Nature of Business','rules'   => 'required'),
			);
		$this->form_validation->set_rules($config);
		if ($this->form_validation->run() == FALSE)
        {
        	...
        	if($nature_of_business_id=='')
        	{
        		$check=$this->db->get_where('nature_of_business',array('name'=>$name))->num_rows();
				if($check==0){
					$data = array('name'=>$name,'is_active'=>1,'created_at'=>$created_at);
					$res=$this->db->insert('nature_of_business',$data);
					$insert_id = $this->db->insert_id();
					$this->admin_logs('NULL',$insert_id,'Add Nature of Business');
					if($res==1){
						$this->session->set_flashdata('success','Nature of Business Saved Successfully...');
					}else{
						$this->session->set_flashdata('failed','This Nature of Business Saved Failed!...');
					}
				}else{
					$this->session->set_flashdata('failed','This Nature of Business Already Existed!...');
				}
        	}else{
        		$check=$this->db->get_where('nature_of_business',array('name'=>$name,'nature_of_business_id!='=>$nature_of_business_id))->num_rows();
				if($check==0){
					$res = $this->db->update('nature_of_business',array('name'=>$name,'updated_at'=>$created_at),array('nature_of_business_id'=>$nature_of_business_id));
					if($res==1){
						$this->session->set_flashdata('success','Nature of Business Updated successfully...');
					}else{
						$this->session->set_flashdata('failed','Nature of Business updated added !...');
					}
				}else{
					$this->session->set_flashdata('failed','This Nature of Business Already Existed!...');
				}
	...
	public function eco_system_practice()
	{
		checklogin_admin('Eco System/Practice');
		$admin_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Designations');
		$data['eco_system_practice']=$this->db->query("SELECT `eco_system_practice_id`, `name`, `is_active`, `created_at`, `updated_at` FROM `eco_system_practice`")->result_array();
		$data['active_menu']='eco_system_practice';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/eco_system_practice');
		$this->load->view('admin/footer');
	}
	public function check_eco_system_practice_existed()
	{
		checklogin_admin('Eco System/Practice');
		$admin_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Eco System/Practice');
		$eco_system_practice_name=trim($this->input->post('eco_system_practice_name'));
		$check=$this->db->get_where('eco_system_practice',array('name'=>$eco_system_practice_name))->num_rows();
		if($check==0){
			$data=array('name'=>$eco_system_practice_name,'is_active'=>1,'created_at'=>date("Y-m-d H:i:s"));
			$res=$this->db->insert('eco_system_practice',$data);
			$dataArr['get_eco_system_practice']=$this->db->query("SELECT `eco_system_practice_id`, `name`, `is_active`, `created_at`, `updated_at` FROM `eco_system_practice` WHERE `is_active`=1")->result_array();
			$dataArr['get_eco_system_practice']['res']=1;
			echo json_encode($dataArr);
		}else{
			$dataArr['get_eco_system_practice']['res']=0;
			echo json_encode($dataArr);
		}
	}
	public function change_eco_system_practice_status($eco_system_practice_id='',$sta='')
	{
		checklogin_admin('Eco System/Practice','Write');
	    $this->admin_logs('NULL',$nature_of_business_id,'Change Eco System/Practice Status',$sta);
		$data = array('is_active'=>$sta);
		$this->db->where('eco_system_practice_id',$eco_system_practice_id);
		$res=$this->db->update('eco_system_practice',$data);
		if ($res==1)
		{
			$this->session->set_flashdata('success','<div class="alert alert-success msgfade"><strong>Eco System/Practice Status Updated Successfully...</strong></div>');
		}
		else
		{
			$this->session->set_flashdata('failed','<div class="alert alert-warning msgfade"><strong>Eco System/Practice Status Update Failed!...</strong></div>');
		}
		redirect('admin/eco_system_practice');
	}
	public function add_eco_system_practice($eco_system_practice_id='')
	{
		checklogin_admin('Eco System/Practice','Write');
		$admin_id=$this->session->userdata('emp_id');
		if($eco_system_practice_id!=''){
			$data['eco_system_practice']=$this->db->query("SELECT `eco_system_practice_id`, `name`, `is_active`, `created_at`, `updated_at` FROM `eco_system_practice` WHERE `eco_system_practice_id`=$eco_system_practice_id")->row_array();
		}
		$data['active_menu']='eco_system_practice';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/add_eco_system_practice');
		$this->load->view('admin/footer');
	}
	public function save_eco_system_practice()
	{
		checklogin_admin('Eco System/Practice');
		$admin_id=$this->session->userdata('emp_id');
		$eco_system_practice_id=$this->input->post('eco_system_practice_id');
		$name=trim($this->input->post('name'));
		$created_at =date("Y-m-d H:i:s");
		$config = array( 
			array( 'field' => 'name', 'label' => 'Nature of Business','rules'   => 'required'),
		...
		if ($this->form_validation->run() == FALSE)
        {
        	...
        	if($eco_system_practice_id=='')
        	{
        		$check=$this->db->get_where('eco_system_practice',array('name'=>$name))->num_rows();
				if($check==0){
					$data = array('name'=>$name,'is_active'=>1,'created_at'=>$created_at);
					$res=$this->db->insert('eco_system_practice',$data);
					$insert_id = $this->db->insert_id();
					$this->admin_logs('NULL',$insert_id,'Add Eco System/Practice');
					if($res==1){
						$this->session->set_flashdata('success','Eco System/Practice Saved Successfully...');
					}else{
						$this->session->set_flashdata('failed','This Eco System/Practice Saved Failed!...');
					}
				}else{
					$this->session->set_flashdata('failed','This Eco System/Practice Already Existed!...');
				}
        	}else{
        		$check=$this->db->get_where('eco_system_practice',array('name'=>$name,'eco_system_practice_id!='=>$eco_system_practice_id))->num_rows();
				if($check==0){
					$res = $this->db->update('eco_system_practice',array('name'=>$name,'updated_at'=>$created_at),array('eco_system_practice_id'=>$eco_system_practice_id));
					if($res==1){
						$this->session->set_flashdata('success','Eco System/Practice Updated Successfully...');
					}else{
						$this->session->set_flashdata('failed','Eco System/Practice Updated Failed!...');
					}
				}else{
					$this->session->set_flashdata('failed','This Eco System/Practice Already Existed!...');
				}
	...
	{
		checklogin_admin('Work Orders');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Work Orders');
		$data['active_menu']='work_order';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/work_orders');
		$this->load->view('admin/footer');
	}
	public function get_work_order_list()
	{
		...
		$records=array();
		$returndata=array();
		$totalcount=$this->loginmodel->search_work_order_Details(1,$start,$length,$search['value'],$column,$dir);
		$returndata['recordsTotal']=0;
		$returndata['recordsFiltered']=0;
		$returndata['recordsTotal']+=$totalcount;
		if($search['value']!=''){
		...
		$stu['output'] = $this->loginmodel->search_work_order_Details(2,$start,$length,$search['value'],$column,$dir);
		$stu['returndata']=$returndata;
		if(is_array($stu['output']) && count($stu['output'])>0){
				foreach($stu['output'] as $k=>$res){
					$NewArr=array();
					$NewArr[]=$k+1;
					$NewArr[]=$res['clinet_name'];
					$NewArr[]=$res['project_name'];
					$NewArr[]=$res['PO'];
					$NewArr[]=$res['no_of_resources'];
					if($res['is_active'] == 0) {
						$status= '<button class="btn btn-danger waves-effect waves-light" onclick="change_work_order_status('.$res['work_orders_id'].',1);"> Inactive </button>';
					}else{
						$status= '<button class="btn btn_success waves-effect waves-light" onclick="change_work_order_status('.$res['work_orders_id'].',0);"> Active </button>';
					}
					$NewArr[]=$status.'| <a href="'.base_url().'admin/edit_work_order/'.$res['work_orders_id'].'" class="btn btn-info waves-effect waves-light"><i class="fa fa-edit" style="color: #fff !important;"></i> </a> | <a href="'.base_url().'admin/view_work_order/'.$res['work_orders_id'].'" class="btn btn-info waves-effect waves-light"><i class="fa fa-eye" style="color: #fff !important;"></i> </a>';
					$returndata['data'][]=$NewArr;
				}
		}else{
			$returndata['data']=array();
	...
	public function add_work_order()
	{
		...
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Candidate Based Rate Config');
		$data['nature_of_business']=$this->db->query("SELECT `nature_of_business_id`, `name`, `is_active`, `created_at`, `updated_at` FROM `nature_of_business` WHERE `is_active`=1")->result_array();
		$data['eco_system_practice']=$this->db->query("SELECT `eco_system_practice_id`, `name`, `is_active`, `created_at`, `updated_at` FROM `eco_system_practice` WHERE `is_active`=1")->result_array();
		$data['Type']='Add';
		$data['active_menu']='work_orders';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/add_work_order');
		$this->load->view('admin/footer');
	}
	public function export_work_orders()
	{
		checklogin_admin('Work Orders');
		$admin_id=$this->session->userdata('emp_id');
		$dt=$this->db->query("SELECT t1.`work_orders_id`, t1.`clinet_name`, t1.`client_details`, t1.`project_name`, t1.`PO_deal_amt`, t1.`PO`, t1.`start_dt`, t1.`end_dt`, t1.`PO_Hrs`, t1.`nature_of_business_id`, t1.`no_of_resources`, t1.`eco_system_practice_id`, t1.`year`, t1.`recognized_amt`, t1.`backlog_amt`, t1.`EMP_contribution`, t1.`is_active`, t1.`is_active_date`, t1.`created_at`, t2.`work_orders_resources_id`, t2.`work_orders_id` as `t2workorderid`, t2.`emp_id`, t2.`emp_per_hrs`, t2.`emp_start_dt`, t2.`emp_end_dt`, t2.`emp_title`, t2.`kpis` FROM `work_orders` as `t1` LEFT JOIN `work_orders_resources` as `t2` ON t1.`work_orders_id`=t2.`work_orders_id`")->result_array();
		$date =date('d-M-Y');
		$name = 'Work_Orders'.$date;
        $name=$name.'.xls';
        header("Content-Type: application/vnd.ms-excel");
		header("Content-disposition: Attachment; filename=$name");
		$aa ="S.No\tClient Name\tClient Details\tProject Name\tPO Deal amount\tPO\tStart Date\tEnd Date\tPO Hrs\tNature of Business\tNo.of Resources\tEco System/Practice\tYear\tRecognized Amt\tBacklog Amt\tEMP Contribution\tResource Employee Name\tResource Employee Per Hr\tResource Start Date\tResource End Date\tResource Employee Title\tResource KPIs\tStatus\tInactive Date\tCreated Date\n";
		$counter=1;
		foreach($dt as $k=>$val){
			if($val['nature_of_business_id']!=''){
		    	$nature_of_business_id=$val['nature_of_business_id'];
				$GetNatureDetails=$this->db->query("SELECT `nature_of_business_id`, `name`, `is_active`, `created_at`, `updated_at` FROM `nature_of_business` WHERE `nature_of_business_id`=$nature_of_business_id")->row_array();
				$NatureDetails= $GetNatureDetails['name'];
			}else{
				$NatureDetails='';
			}
			if($val['eco_system_practice_id']!=''){
		    	$eco_system_practice_id=$val['eco_system_practice_id'];
				$GetEcoDetails=$this->db->query("SELECT `eco_system_practice_id`, `name`, `is_active`, `created_at`, `updated_at` FROM `eco_system_practice` WHERE `eco_system_practice_id`=$eco_system_practice_id")->row_array();
				$EcoDetails= $GetEcoDetails['name'];
			}else{
				$EcoDetails='';
			}
		    if($val['emp_id']!=''){
		    	$emp_id=$val['emp_id'];
				$GetEmpDetails=$this->db->query("SELECT `emp_id`, `fname`, `lname`, `email_id`, `emp_code` FROM `employees` WHERE `emp_id`=$emp_id")->row_array();
				$empName= $GetEmpDetails['fname'].' '.$GetEmpDetails['lname'].'_'.$GetEmpDetails['emp_code'];
			}else{
				$empName='';
			}
			...
			if($val['start_dt']==''){
				...
				$start_dt=DD_M_YY_h_i_s($val['start_dt']);
			}
			if($val['end_dt']==''){
				$end_dt='';
			}else{
				$end_dt=DD_M_YY_h_i_s($val['end_dt']);
			}
			if($val['emp_start_dt']==''){
				$emp_start_dt='';
			}else{
				$emp_start_dt=DD_M_YY_h_i_s($val['emp_start_dt']);
			}
			if($val['emp_end_dt']==''){
				$emp_end_dt='';
			}else{
				$emp_end_dt=DD_M_YY_h_i_s($val['emp_end_dt']);
			}
			if($val['is_active_date']==''){
				$is_active_date='';
			}else{
				$is_active_date=DD_M_YY_h_i_s($val['is_active_date']);
			}
			 $aa.=$counter."\t".$val['clinet_name']."\t".$val['client_details']."\t".$val['project_name']."\t".$val['PO_deal_amt']."\t".$val['PO']."\t".$start_dt."\t".$end_dt."\t".$val['PO_Hrs']."\t".$NatureDetails."\t".$val['no_of_resources']."\t".$EcoDetails."\t".$val['year']."\t".$val['recognized_amt']."\t".$val['backlog_amt']."\t".$val['EMP_contribution']."\t".$empName."\t".$val['emp_per_hrs']."\t".$emp_start_dt."\t".$emp_end_dt."\t".$val['emp_title']."\t".$val['kpis']."\t".$Status."\t".$is_active_date."\t".DD_M_YY_h_i_s($val['created_at'])."\n";
             $counter++;
        }
        echo $aa;
	}
	public function ajax_work_resources()
	{
		checklogin_admin('Work Orders');
		$emp_id=$this->session->userdata('emp_id');
		$work_orders_id=$this->input->post('work_orders_id');
		$data['work_orders_id']=$work_orders_id;
		$clinet_name=$this->input->post('clinet_name');
		...
		$data['GetRolesAccess']=$this->read_write('Work Orders');
		$data['employees']=$this->db->query("SELECT `emp_id`, `fname`, `lname`, `gender`, `dob`, `emp_code` FROM `employees`")->result_array();
		if($this->input->post('work_orders_id')!=''){
			$get_work_order_resources=$this->db->query("SELECT `work_orders_resources_id`, `work_orders_id`, `emp_id`, `emp_per_hrs`, `emp_start_dt`, `emp_end_dt`, `emp_title`, `kpis`, `is_active`, `created_at`, `updated_at` FROM `work_orders_resources` WHERE `work_orders_id`=$work_orders_id")->result_array();
			if(!empty($get_work_order_resources)){
				if($type==''){
					$data['work_order_resources']=$get_work_order_resources;
					echo $this->load->view('admin/ajax_work_resources',$data,TRUE);
				}else{
					$data['work_order_resources']=$get_work_order_resources;
					echo $this->load->view('admin/view_ajax_work_resources',$data,TRUE);
				}
			}else{
				$data['work_order_resources']=array();
				echo $this->load->view('admin/ajax_work_resources',$data,TRUE);
			}
		}else{
			echo 0;
	...
	public function add_ajax_work_resources()
	{
		checklogin_admin('Work Orders');
		$emp_id=$this->session->userdata('emp_id');
		$work_orders_id=$this->input->post('work_orders_id');
	...
	public function ajax_save_work_resources()
	{
		checklogin_admin('Work Orders');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Work Orders');
		$work_orders_id=$this->input->post('work_orders_id');
		$emp_id=$this->input->post('emp_id[]');
		$emp_per_hrs=$this->input->post('emp_per_hrs[]');
		$emp_start_dt=$this->input->post('emp_start_dt[]');
		$emp_end_dt=$this->input->post('emp_end_dt[]');
		$emp_title=$this->input->post('emp_title[]');
		$kpis=$this->input->post('kpis[]');
		if($work_orders_id=='' || empty($emp_id) || empty($emp_per_hrs) || empty($emp_start_dt) || empty($emp_end_dt) || empty($emp_title) || empty($kpis))
		{
			echo json_encode(array('status'=>'error'));
		}else{
			$this->db->where('work_orders_id',$work_orders_id);
			$this->db->delete('work_orders_resources');
			foreach($emp_id as $key => $empArr)
			{
				$ArrInsert=array('work_orders_id'=>$work_orders_id,'emp_id'=>$empArr,'emp_per_hrs'=>$emp_per_hrs[$key],'emp_start_dt'=>YY_MM_DD($emp_start_dt[$key]),'emp_end_dt'=>YY_MM_DD($emp_end_dt[$key]),'emp_title'=>$emp_title[$key],'kpis'=>$kpis[$key],'is_active'=>1,'created_at'=>date("Y-m-d H:i:s"));
				$res=$this->db->insert('work_orders_resources',$ArrInsert);
			}
			$Get_no_of_resources=$this->db->query("SELECT `work_orders_resources_id`, `work_orders_id`, `emp_id`, `emp_per_hrs`, `emp_start_dt`, `emp_end_dt`, `emp_title`, `kpis` FROM `work_orders_resources` WHERE `work_orders_id`=$work_orders_id")->num_rows();
			if($res==1)
			{
				echo json_encode(array('status'=>1,'no_of_resources'=>$Get_no_of_resources));
			}else{
				echo json_encode(array('status'=>0,'no_of_resources'=>''));
			}
		}
	}
	public function ajax_save_work_order()
	{
		checklogin_admin('Work Orders');
		$admin_id=$this->session->userdata('emp_id');
		$work_orders_id=$this->input->post('work_orders_id');
		$GetInputName=trim($this->input->post('InputName'));
		$Val=$this->input->post('Val');
		if($GetInputName=='start_dt' || $GetInputName=='end_dt'){
			$InputName=YY_MM_DD($GetInputName);
		}else{
		...
		if(@$InputName=='' || @$Val==''){
        	...
        	if($work_orders_id==''){
        		$ArrInsert=array($InputName=>$Val,'is_active'=>1,'created_at'=>$created_date);
				$res=$this->db->insert('work_orders',$ArrInsert);
				$insert_id = $this->db->insert_id();
				$this->admin_logs('NULL',$insert_id,'Save Work Orders');
        	}else{
        		$ArrInsert=array($InputName=>$Val,'is_active'=>1,'created_at'=>$created_date);
        		$this->db->where('work_orders_id',$work_orders_id);
				$res=$this->db->update('work_orders',$ArrInsert);
				$insert_id = $work_orders_id;
				$this->admin_logs('NULL',$work_orders_id,'Update Work Orders');
        	}
			if($res==1){
				echo $insert_id;
			}else{
	...
	public function save_work_order()
	{
		...
		$created_date =date("Y-m-d H:i:s");
		$config = array( 
			array( 'field'   => 'clinet_name', 'label'   => 'Clinet Name','rules'   => 'required'),
			array( 'field'   => 'project_name', 'label'   => 'Project Name','rules'   => 'required'),
			array( 'field'   => 'PO_deal_amt', 'label'   => 'PO Deal Amt','rules'   => 'required'),
			array( 'field'   => 'PO', 'label'   => 'PO','rules'   => 'required'),
			array( 'field'   => 'start_dt', 'label'   => 'Start Date','rules'   => 'required'),
			array( 'field'   => 'end_dt', 'label'   => 'End Date','rules'   => 'required'),
			array( 'field'   => 'PO_Hrs', 'label'   => 'PO Hrs','rules'   => 'required'),
			array( 'field'   => 'nature_of_business_id', 'label'   => 'Nature Of Business','rules'   => 'required'),
			array( 'field'   => 'no_of_resources', 'label'   => 'No Of Resources','rules'   => 'required'),
			array( 'field'   => 'eco_system_practice_id', 'label'   => 'Eco System Practice','rules'   => 'required'),
			array( 'field'   => 'year', 'label'   => 'Year','rules'   => 'required'),
			array( 'field'   => 'recognized_amt', 'label'   => 'Recognized Amt','rules'   => 'required'),
			array( 'field'   => 'backlog_amt', 'label'   => 'Backlog Amt','rules'   => 'required'),
			array( 'field'   => 'EMP_contribution', 'label'   => 'EMP Contribution','rules'   => 'required'),
		);
		$this->form_validation->set_rules($config);
		if ($this->form_validation->run() == FALSE)
        {
        ...
        {	
        	if($post['work_orders_id']=='')
        	{
        		$images = array();
				if(isset($_FILES['simage']['name']))
				{
		        	foreach ($_FILES['simage']['name'] as $key => $image)
		        	{
			            $_FILES['images']['name']= $_FILES['simage']['name'][$key];
			            $_FILES['images']['type']= $_FILES['simage']['type'][$key];
			            $_FILES['images']['tmp_name']= $_FILES['simage']['tmp_name'][$key];
			            $_FILES['images']['error']= $_FILES['simage']['error'][$key];
			            $_FILES['images']['size']= $_FILES['simage']['size'][$key];
			            $fileName = strtolower(pathinfo($image, PATHINFO_EXTENSION));
			            if($fileName=='jpeg' || $fileName=='jpg' || $fileName=='png' || $fileName=='pdf' || $fileName=='csv' || $fileName=='xlsx' || $fileName=='xls')
			            {
			                
			            }else{
			            	$this->session->set_flashdata('failed','<div class="alert alert-danger msgfade"><strong>Please upload jpg, jpeg, png, pdf, csv, xlsx, xls formates only</strong></div>');
			                redirect('admin/work_orders');
			            }
			        }
			    }
        	}
		    $post['start_dt']=YY_MM_DD($post['start_dt']);
		    $post['end_dt']=YY_MM_DD($post['end_dt']);
	        $post['created_at']=$created_date;
	        $post['is_active']=1;
	        $this->db->where('work_orders_id',$post['work_orders_id']);
			$res=$this->db->update('work_orders',$post);
			if(!empty($_FILES['simage']['name']))
			{
				foreach ($_FILES['simage']['name'] as $key => $image)
		        {
		            ...
		            $images[] = str_replace(" ","_",$fileName);
		            $images[] = $filePath;
		            $config['upload_path'] = 'assets/work_orders';
		            $config['file_name'] = $filePath;
		            $config['allowed_types'] = '*';
		            $config['overwrite']=true;
		            $this->load->library('upload');
	            	...
	            	if($fileName!='')
	            	{
	            		$ArrImgInsert =array('work_orders_id'=>$post['work_orders_id'],'file_name'=>$fileName,'file_path'=>$filePath,'is_active'=>1,'created_at'=>$created_date);
	                	$this->db->insert('work_orders_documents11',$ArrImgInsert);
	            	}
	    		}
			}
			...
			if($res==1){
				...
				$this->session->set_flashdata('failed','This Work Order Added Failed...');
			}
		}
		redirect('admin/work_orders');
	}
	public function edit_work_order($work_orders_id='')
	{
		checklogin_admin('Work Orders');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Work Orders');
		$data['Type']='Edit';
		$data['work_orders_id']=$work_orders_id;
		$data['work_orders']=$this->db->query("SELECT `work_orders_id`, `clinet_name`, `client_details`, `project_name`, `PO_deal_amt`, `PO`, `start_dt`, `end_dt`, `PO_Hrs`, `nature_of_business_id`, `no_of_resources`, `eco_system_practice_id`, `year`, `recognized_amt`, `backlog_amt`, `EMP_contribution`, `is_active`, `is_active_date`, `created_at`, `updated_at` FROM `work_orders` WHERE `work_orders_id`=$work_orders_id")->row_array();
		$data['work_orders_documents']=$this->db->query("SELECT `work_orders_document_id`, `work_orders_id`, `file_name`, `file_path`, `is_active`, `created_at`, `updated_at` FROM `work_orders_documents` WHERE `work_orders_id`=$work_orders_id")->result_array();
		$data['work_orders_resources']=$this->db->query("SELECT `work_orders_resources_id`, `work_orders_id`, `emp_id`, `emp_per_hrs`, `emp_start_dt`, `emp_end_dt`, `emp_title`, `kpis`, `is_active`, `created_at`, `updated_at` FROM `work_orders_resources` WHERE `work_orders_id`=$work_orders_id")->result_array();
		$data['nature_of_business']=$this->db->query("SELECT `nature_of_business_id`, `name`, `is_active`, `created_at`, `updated_at` FROM `nature_of_business` WHERE `is_active`=1")->result_array();
		$data['eco_system_practice']=$this->db->query("SELECT `eco_system_practice_id`, `name`, `is_active`, `created_at`, `updated_at` FROM `eco_system_practice` WHERE `is_active`=1")->result_array();
		$data['active_menu']='work_order';
		$this->load->view('admin/menu',$data);
	...
	public function view_work_order($work_orders_id='')
	{
		checklogin_admin('Work Orders');
		$emp_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Work Orders');
		...
		$data['active_menu']='work_order';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/view_work_order');
		$this->load->view('admin/footer');
	}
	public function change_work_order_status($work_orders_id='',$sta='')
	{
	    checklogin_admin('Work Orders','Write');
	    $emp_id=$this->session->userdata('emp_id');
	    $this->admin_logs($emp_id,'NULL','Change Work Orders Status',$sta);
		$data = array('is_active'=>$sta,'is_active_date'=>date("Y-m-d H:i:s"));
		$this->db->where('work_orders_id',$work_orders_id);
		$res=$this->db->update('work_orders',$data);
		if ($res==1)
		{
			$this->session->set_flashdata('success','<div class="alert alert-success msgfade"><strong>Work Order Status Updated Successfully...</strong></div>');
		}
		else
		{
			$this->session->set_flashdata('failed','<div class="alert alert-warning msgfade"><strong>Work Order Status Update Failed!...</strong></div>');
		}
		redirect('admin/work_orders');
	}
	public function delete_work_orders_documents()
	{
	    checklogin_admin('Work Orders','Write');
	    $emp_id=$this->session->userdata('emp_id');
	    $work_orders_document_id=$this->input->post('work_orders_document_id');
	    $this->admin_logs('NULL',$work_orders_document_id,'Deleted Work Orders Documents');
		$this->db->where('work_orders_document_id',$work_orders_document_id);
		$res=$this->db->delete('work_orders_documents');
		if($res==1)
		{
			echo 1;
		}
	...
	public function asset_logs($changed_id='',$type='',$sta='')
	{
		checklogin_admin();
		$admin_id=$this->session->userdata('emp_id');
		if($admin_id!=''){
		...
		$ip = $this->input->ip_address();
		$date =date("Y-m-d H:i:s");
		$ArrData=array('admin_id'=>$admin_id,'admin_name'=>$empName,'changed_id'=>$changed_id,'ip_address'=>$ip,'type'=>$type,'status'=>$sta,'created_at'=>$date);
		$this->db->insert('asset_logs',$ArrData);
	}
	...
	public function get_assets_list()
	{
		...
		if(is_array($stu['output']) && count($stu['output'])>0){
				foreach($stu['output'] as $k=>$res){
					...
					$NewArr[]=$k+1;
					$NewArr[]=$res['t1name'];
					$NewArr[]=$res['asset_tag'];
					$NewArr[]=$res['aname'];
					$NewArr[]=$res['bname'];
					$NewArr[]=$res['lname'];
					if($res['emp_id']==''){
						$NewArr[]='<button class="btn btn_success waves-effect waves-light" onclick="assign_asset('.$res['asset_id'].',1);"><i class="fa fa-user" style="color: #fff !important;"></i> Assign </button>';
					}else{
						$emp_id=$res['emp_id'];
						$GetEmpDetails=$this->db->query("SELECT `emp_id`, `fname`, `lname`, `email_id`, `emp_code` FROM `employees` WHERE `emp_id`=$emp_id")->row_array();
						$NewArr[]= $GetEmpDetails['fname'].' '.$GetEmpDetails['lname'].' ('.$GetEmpDetails['emp_code'].')';
					}
					if($res['emp_id']==0 || $res['emp_id']==''){
						$DeassignBtn='';
					}else{
						$DeassignBtn='<a class="dropdown-item text-danger" href="javascript:void(0);" onclick="Deassign('.$res['asset_id'].',2);">Deassign</a>';
					}
					
					$NewArr[]='<div class="app-new-supplier"><a href="'.base_url().'admin/edit_asset/'.$res['asset_id'].'" class="btn btn-info waves-effect waves-light"><i class="fa fa-edit" style="color: #fff !important;"></i> Edit </a> <div class="dropdown"><button class="btn new-supplier btn-info dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-ellipsis-v" aria-hidden="true"></i></button><div class="dropdown-menu" aria-labelledby="dropdownMenuButton">'.$DeassignBtn.'<a class="dropdown-item" href="#">View</a></div></div></div>';
					$returndata['data'][]=$NewArr;
				}
		}else{
			$returndata['data']=array();
	...
	public function ajax_assign_asset()
	{
		checklogin_admin('Asset');
		$admin_id=$this->session->userdata('emp_id');
		$asset_id=$this->input->post('asset_id');
		$type=$this->input->post('type');
		if($asset_id!=''){
			$data['asset']=$this->db->query("SELECT `asset_id`, `name`, `asset_tag`, `brand_id` FROM `assets` WHERE `asset_id`=$asset_id")->row_array();
			$brand_id=$data['asset']['brand_id'];
			$data['brand']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='brand' AND `id`=$brand_id")->row_array();
			$data['asset_id']=$asset_id;
			$data['type']=$type;
			$data['asset_tag']=$data['asset']['asset_tag'];
			$data['brand_id']=$data['brand']['id'];
			$data['employees']=$this->db->query("SELECT `emp_id`, `fname`, `lname`, `email_id`, `emp_code` FROM `employees`")->result_array();
			echo $this->load->view('admin/ajax_assign_asset',$data,true);
		}else{
			echo 0;
		}
	}
	public function ajax_save_asset_assign()
	{
		checklogin_admin('Asset');
		$admin_id=$this->session->userdata('emp_id');
		if($admin_id==''){
		...
		$type=$this->input->post('type');
		$emp_id=$this->input->post('emp_id');
		$assign_dt=YY_MM_DD($this->input->post('assign_dt'));
		$cmnts=$this->input->post('cmnts');
		$maintaincecmnts=$this->input->post('maintaincecmnts');
		$stus=$this->input->post('stus');
		if($type==1)
		{
			if($asset_id!='' || $emp_id!='' || $assign_dt!='')
			{
				$Arr=array('emp_id'=>$emp_id,'assigned_dt'=>$assign_dt,'comments'=>$cmnts,'asset_status'=>'Assigned');
				$this->db->where('asset_id',$asset_id);
				$this->db->update('assets',$Arr);
				$ArrTwo=array('asset_id'=>$asset_id,'emp_id'=>$emp_id,'assigned_dt'=>$assign_dt,'comments'=>$cmnts,'id'=>$id,'is_active'=>1,'created_at'=>date('Y-m-d H:i:s'),'asset_status'=>'Assigned');
				$res=$this->db->insert('asset_assigned_logs',$ArrTwo);
				echo $res;
			}else{
				echo 0;
			}
		}else{
			if($asset_id!='' || $emp_id!='' || $assign_dt!='' || $stus!='')
			{
				$GetEmpid=$this->db->get_where('assets', array('asset_id='=>$asset_id))->row_array();
				$employee_id=$GetEmpid['emp_id'];
				if($stus=='Deassigned'){
					$Arr=array('emp_id'=>'','deassigned_dt'=>$assign_dt,'deassigned_remarks'=>$cmnts,'asset_status'=>$emp_id);
					$this->db->where('asset_id',$asset_id);
					$this->db->update('assets',$Arr);
				}else if($stus=='Deassigned&Maintance'){
					$Arr=array('deassigned_dt'=>$assign_dt,'deassigned_remarks'=>$cmnts,'asset_status'=>$emp_id);
					$this->db->where('asset_id',$asset_id);
					$this->db->update('assets',$Arr);
					$MaintanceArr=array('asset_id'=>$asset_id,'supplier_id'=>'','type_id'=>'','start_dt'=>date("Y-m-d"),'comments'=>@$maintaincecmnts,'created_at'=>date("Y-m-d H:i:s"));
					$this->db->insert('asset_maintenance',$MaintanceArr);
				}else{
					$Arr=array('deassigned_dt'=>$assign_dt,'deassigned_remarks'=>$cmnts,'asset_status'=>$stus);
					$this->db->where('asset_id',$asset_id);
					$this->db->update('assets',$Arr);
				}
				$ArrTwo=array('asset_id'=>$asset_id,'emp_id'=>$employee_id,'assigned_dt'=>$assign_dt,'comments'=>$cmnts,'id'=>$id,'is_active'=>1,'created_at'=>date('Y-m-d H:i:s'),'asset_status'=>$stus);
				$res=$this->db->insert('asset_assigned_logs',$ArrTwo);
				echo 2;
			}else{
				echo 0;
	...
	public function generate_asset_tag($assettag='')
	{
		checklogin_admin('Asset');
		$admin_id=$this->session->userdata('emp_id');
		if($assettag==''){
			$generate_asset_tag=mt_rand(1000000,9999999);
			$dtseconds=date('s');
			$asset_tag='VT'.$generate_asset_tag.$dtseconds;
		}else{
			$asset_tag=$assettag;
		}
		$ChkTag=$this->db->get_where('assets', array('asset_tag='=>'$asset_tag'))->num_rows();
		if($ChkTag==0){
			return $asset_tag;
		}else{
			$this->generate_asset_tag();
		}
	}
	public function add_asset()
	{
		checklogin_admin('Asset','Write');
		$admin_id=$this->session->userdata('emp_id');
		$data['asset_tag']=$this->generate_asset_tag();
		$data['suppliers']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='supplier' AND `is_active`=1")->result_array();
		$data['locations']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='location' AND `is_active`=1")->result_array();
		$data['brands']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='brand' AND `is_active`=1")->result_array();
		$data['assettypes']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='assettype' AND `is_active`=1")->result_array();
		$data['status']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='status' AND `is_active`=1")->result_array();
		$data['active_menu']='asset';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/add_asset');
		$this->load->view('admin/footer');
	}
	public function ajax_save_asset_fields()
	{
		checklogin_admin('Asset');
		$admin_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Asset');
		$val=trim($this->input->post('val'));
		$typ=trim($this->input->post('type'));
		if($typ=='asset type'){
			$type='assettype';
		}else{
			$type=$typ;
		}
		$check=$this->db->get_where('asset_items',array('name'=>$val,'type'=>$type))->num_rows();
		if($check==0){
			$data=array('name'=>$val,'type'=>$type,'is_active'=>1,'created_at'=>date("Y-m-d H:i:s"));
			$res=$this->db->insert('asset_items',$data);
			$dataArr['output']=$this->db->get_where('asset_items',array('type'=>$type,'is_active'=>1))->result_array();
			$dataArr['output']['res']=1;
			echo json_encode($dataArr);
		}else{
			$dataArr['output']['res']=0;
			echo json_encode($dataArr);
		}
	}
	public function save_asset()
	{
		...
		$config = array( 
			array( 'field'=> 'name', 'label'=>'Name','rules'=> 'required'),
			array( 'field'=> 'asset_tag', 'label'=>'Asset Tag','rules'=> 'required'),
			array( 'field'=> 'supplier_id', 'label'=>'Supplier','rules'=> 'required'),
			array( 'field'=> 'location_id', 'label'=>'Location','rules'=> 'required'),
			array( 'field'=> 'brand_id', 'label'=>'Brand','rules'=> 'required'),
			array( 'field'=> 'serial_no', 'label'=>'Serial No','rules'=> 'required'),
			array( 'field'=> 'asset_type_id', 'label'=>'Asset Type','rules'=> 'required'),
			array( 'field'=> 'cost', 'label'=>'Cost','rules'=> 'required'),
        ...
        else
        {
	        $post['created_at']=$created_date;
	        $post['is_active']=1;
	        $post['purchase_date']=YY_MM_DD($post['purchase_date']);
		        if(@$post['asset_id']==''){
		        	if($post['asset_tag']!=''){
		        	$code = $post['asset_tag'];
		        }else{
		        	$code = $this->generate_asset_tag();
		        }
				$this->load->library('zend');
				$this->zend->load('Zend/Barcode');
				$imageResource = Zend_Barcode::factory('code128', 'image', array('text'=>$code,'barHeight' => 20,'fontSize'=>12), array())->draw();
				imagepng($imageResource, 'barcodes/'.$code.'.png');
				$data['barcode'] = 'barcodes/'.$code.'.png';
	        	$res = $this->db->insert('assets',$post);
	        	$insert_id = $this->db->insert_id();
				$this->asset_logs($insert_id,'Add Asset');
				if($res==1){
					$this->session->set_flashdata('success','Asset Saved Successfully...');
				}else{
					$this->session->set_flashdata('failed','Asset Saved Failed...');
				}
	        }else{
	        	$this->db->where('asset_id',$post['asset_id']);
	        	$res = $this->db->update('assets',$post);
				$this->asset_logs($post['asset_id'],'Update Asset');
				if($res==1){
					$this->session->set_flashdata('success','Asset Updated Successfully...');
				}else{
					$this->session->set_flashdata('failed','Asset Updated Failed...');
				}
	        }
	        if(@$post['asset_id']==''){
	        	$assetid=$insert_id;
	        ...
	        if(!empty($_FILES['simage']['name']))
			{
				foreach ($_FILES['simage']['name'] as $key => $image)
		        {
		            ...
		            $images[] = str_replace(" ","_",$fileName);
		            $images[] = $filePath;
		            $config['upload_path'] = 'assets/asset_images';
		            $config['file_name'] = $filePath;
		            $config['allowed_types'] = '*';
		            $config['overwrite']=true;
		            $this->load->library('upload');
	            	...
	            	if($fileName!='')
	            	{
	            		$ArrImgInsert =array('asset_id'=>$assetid,'img_name'=>$fileName,'img_path'=>$filePath,'is_active'=>1,'created_at'=>$created_date);
	                	$this->db->insert('asset_documents',$ArrImgInsert);
	            	}
	    		}
			}
		}
	...
	public function edit_asset($asset_id='')
	{
		checklogin_admin('Asset','Write');
		$admin_id=$this->session->userdata('emp_id');
		$data['asset']=$this->db->query("SELECT `asset_id`, `name`, `asset_tag`, `supplier_id`, `location_id`, `brand_id`, `serial_no`, `asset_type_id`, `cost`, `purchase_date`, `warranty`, `status_id`, `description`, `emp_id`, `assigned_dt`, `asset_status`, `comments`, `is_active`, `created_at`, `updated_at` FROM `assets` WHERE `asset_id`=$asset_id")->row_array();
		$data['suppliers']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='supplier' AND `is_active`=1")->result_array();
		$data['locations']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='location' AND `is_active`=1")->result_array();
		$data['brands']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='brand' AND `is_active`=1")->result_array();
		$data['assettypes']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='assettype' AND `is_active`=1")->result_array();
		$data['status']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='status' AND `is_active`=1")->result_array();
		$data['documents']=$this->db->query("SELECT `asset_document_id`, `asset_id`, `img_name`, `img_path`, `is_active`, `created_at`, `updated_at` FROM `asset_documents` WHERE `asset_id`=$asset_id")->result_array();
		$data['asset_id']=$asset_id;
		$data['active_menu']='assets';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/add_asset');
	...
	public function delete_asset_pictures()
	{
	    checklogin_admin('Asset','Write');
	    $emp_id=$this->session->userdata('emp_id');
	    $asset_document_id=$this->input->post('asset_document_id');
	    $GetPicturePath=$this->db->query("SELECT `asset_document_id`, `asset_id`, `img_name`, `img_path`, `is_active`, `created_at`, `updated_at` FROM `asset_documents` WHERE `asset_document_id`=$asset_document_id")->row_array();
	    $this->asset_logs('NULL',$asset_document_id,'Deleted Asset Picture');
		$this->db->where('asset_document_id',$asset_document_id);
		$res=$this->db->delete('asset_documents');
		if($res==1)
		{
			unlink("assets/asset_images/".$GetPicturePath['img_path']);
			echo 1;
		}
		else
		{
	...
	{
		checklogin_admin('Asset Components');
		$admin_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Asset');
		$data['active_menu']='components';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/asset_components');
		$this->load->view('admin/footer');
	}
	public function get_components_list()
	{
		...
		$records=array();
		$returndata=array();
		$totalcount=$this->loginmodel->search_components_Details(1,$start,$length,$search['value'],$column,$dir);
		$returndata['recordsTotal']=0;
		$returndata['recordsFiltered']=0;
		$returndata['recordsTotal']+=$totalcount;
		if($search['value']!=''){
		...
		$stu['output'] = $this->loginmodel->search_components_Details(2,$start,$length,$search['value'],$column,$dir);
		$stu['returndata']=$returndata;
		if(is_array($stu['output']) && count($stu['output'])>0){
				foreach($stu['output'] as $k=>$res){
					$NewArr=array();
					...
					$NewArr[]=$res['qty'];
					$NewArr[]=$res['lname'];
					$NewArr[]='<a href="'.base_url().'admin/edit_component/'.$res['component_id'].'" class="btn btn-info waves-effect waves-light"><i class="fa fa-edit" style="color: #fff !important;"></i> Edit </a>';
					$returndata['data'][]=$NewArr;
				}
		}else{
			$returndata['data']=array();
	...
	{
		checklogin_admin('Asset Components','Write');
		$admin_id=$this->session->userdata('emp_id');
		$data['asset_tag']=$this->generate_asset_tag();
		$data['assets']=$this->db->query("SELECT `asset_id`, `name`, `asset_tag` FROM `assets`")->result_array();
		$data['suppliers']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='supplier' AND `is_active`=1")->result_array();
		$data['locations']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='location' AND `is_active`=1")->result_array();
		$data['brands']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='brand' AND `is_active`=1")->result_array();
		$data['assettypes']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='assettype' AND `is_active`=1")->result_array();
		...
		$data['active_menu']='components';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/add_asset_component');
		$this->load->view('admin/footer');
	}
	public function save_component()
	{
        ...
        else
        {
	        ...
	        if(@$post['component_id']==''){
	        	$res = $this->db->insert('asset_components',$post);
	        	$insert_id = $this->db->insert_id();
				$this->asset_logs($insert_id,'Add Component');
				if($res==1){
					$this->session->set_flashdata('success','Component Saved  Successfully...');
				}else{
					$this->session->set_flashdata('failed','Component Saved Failed...');
				}
	        }else{
	        	$this->db->where('component_id',$post['component_id']);
	        	$res = $this->db->update('asset_components',$post);
				$this->asset_logs($post['component_id'],'Update Component');
				if($res==1){
					$this->session->set_flashdata('success','Component Updated Successfully...');
				}else{
					$this->session->set_flashdata('failed','Component Updated Failed...');
				}
		    }
		    if(@$post['component_id']==''){
	        	$componentid=$insert_id;
	        }else{
		    ...
		    if(!empty($_FILES['simage']['name']))
			{
				foreach ($_FILES['simage']['name'] as $key => $image)
		        {
		            ...
		            $images[] = str_replace(" ","_",$fileName);
		            $images[] = $filePath;
		            $config['upload_path'] = 'assets/asset_component_images';
		            $config['file_name'] = $filePath;
		            $config['allowed_types'] = '*';
		            $config['overwrite']=true;
		            $this->load->library('upload');
	            	...
	            	if($fileName!='')
	            	{
	            		$ArrImgInsert =array('component_id'=>$componentid,'img_name'=>$fileName,'img_path'=>$filePath,'is_active'=>1,'created_at'=>$created_date);
	                	$this->db->insert('asset_component_documents',$ArrImgInsert);
	            	}
	    		}
			}
		}
	...
	public function edit_component($component_id='')
	{
		checklogin_admin('Asset Components','Write');
		$admin_id=$this->session->userdata('emp_id');
		$data['component']=$this->db->query("SELECT `component_id`, `asset_id`, `name`, `serial_no`, `qty`, `supplier_id`, `asset_type_id`, `location_id`, `brand_id`, `cost`, `purchase_date`, `warranty`, `status_id`, `description`, `is_active`, `created_at`, `updated_at` FROM `asset_components` WHERE `component_id`=$component_id")->row_array();
		$data['assets']=$this->db->query("SELECT `asset_id`, `name`, `asset_tag` FROM `assets`")->result_array();
		$data['suppliers']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='supplier' AND `is_active`=1")->result_array();
		$data['locations']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='location' AND `is_active`=1")->result_array();
		$data['brands']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='brand' AND `is_active`=1")->result_array();
		$data['assettypes']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='assettype' AND `is_active`=1")->result_array();
		$data['status']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='status' AND `is_active`=1")->result_array();
		$data['documents']=$this->db->query("SELECT `component_document_id`, `component_id`, `img_name`, `img_path`, `is_active`, `created_at`, `updated_at` FROM `asset_component_documents` WHERE `component_id`=$component_id")->result_array();
	...
	public function change_component_status($component_id='',$sta='')
	{
		checklogin_admin('Asset Components','Write');
	    $this->admin_logs('NULL',$component_id,'Change Asset Status',$sta);
		$data = array('is_active'=>$sta);
	...
	public function delete_asset_component_pictures()
	{
	    checklogin_admin('Asset Components','Write');
	    $emp_id=$this->session->userdata('emp_id');
	    $component_document_id=$this->input->post('component_document_id');
	    $GetPicturePath=$this->db->query("SELECT `component_document_id`, `img_name`, `img_path` FROM `asset_component_documents` WHERE `component_document_id`=$component_document_id")->row_array();
	    $this->asset_logs('NULL',$component_document_id,'Deleted Asset Component Picture');
		$this->db->where('component_document_id',$component_document_id);
		$res=$this->db->delete('asset_component_documents');
		if($res==1)
		{
			unlink("assets/asset_component_images/".$GetPicturePath['img_path']);
			echo 1;
		}
		else
		{
	...
	{
		checklogin_admin('Asset Maintenance');
		$admin_id=$this->session->userdata('emp_id');
		$data['GetRolesAccess']=$this->read_write('Asset');
		$data['active_menu']='maintenances';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/asset_maintenances');
		$this->load->view('admin/footer');
	}
	public function get_maintenances_list()
	{
		$start=$this->input->get('start')?intval( $this->input->get('start') ) :0;
		$length=$this->input->get('length')?intval( $this->input->get('length') ) :0;
		$search=$this->input->get('search')?$this->input->get('search'):array('value'=>'');
		...
		$records=array();
		$returndata=array();
		$totalcount=$this->loginmodel->search_maintenances_Details(1,$start,$length,$search['value'],$column,$dir);
		$returndata['recordsTotal']=0;
		$returndata['recordsFiltered']=0;
		$returndata['recordsTotal']+=$totalcount;
		if($search['value']!=''){
		...
		$stu['output'] = $this->loginmodel->search_maintenances_Details(2,$start,$length,$search['value'],$column,$dir);
		$stu['returndata']=$returndata;
		if(is_array($stu['output']) && count($stu['output'])>0){
				foreach($stu['output'] as $k=>$res){
					$NewArr=array();
					...
					$NewArr[]=$res['sname'];
					$NewArr[]=$res['tname'];
					$NewArr[]=DD_M_YY($res['start_dt']);
					$NewArr[]=DD_M_YY($res['end_dt']);
					$NewArr[]='<a href="'.base_url().'admin/edit_maintenance/'.$res['maintenance_id'].'" class="btn btn-info waves-effect waves-light"><i class="fa fa-edit" style="color: #fff !important;"></i> Edit </a>';
					$returndata['data'][]=$NewArr;
				}
		}else{
			$returndata['data']=array();
	...
	{
		checklogin_admin('Asset Maintenance','Write');
		$admin_id=$this->session->userdata('emp_id');
		$data['assets']=$this->db->query("SELECT `asset_id`, `name`, `asset_tag` FROM `assets`")->result_array();
		$data['suppliers']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='supplier' AND `is_active`=1")->result_array();
		$data['types']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='type' AND `is_active`=1")->result_array();
		$data['active_menu']='maintenances';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/add_asset_maintenance');
		$this->load->view('admin/footer');
	}
	public function save_maintenance()
	{
		...
		$created_date =date("Y-m-d H:i:s");
		$config = array( 
			array( 'field'=> 'asset_id', 'label'=>'Asset','rules'=> 'required'),
			array( 'field'=> 'supplier_id', 'label'=>'Supplier','rules'=> 'required'),
			array( 'field'=> 'type_id', 'label'=>'Type','rules'=> 'required'),
			array( 'field'=> 'start_dt', 'label'=>'Start Date','rules'=> 'required'),
			array( 'field'=> 'end_dt', 'label'=>'End Date','rules'=> 'required'),
		);
		$this->form_validation->set_rules($config);
        ...
        else
        {
	        ...
	        if(@$post['maintenance_id']==''){
	        	$res = $this->db->insert('asset_maintenance',$post);
	        	$insert_id = $this->db->insert_id();
				$this->asset_logs($insert_id,'Add Maintenance');
				if($res==1){
					$this->session->set_flashdata('success','Maintenance Saved  Successfully...');
				}else{
					$this->session->set_flashdata('failed','Maintenance Saved Failed...');
				}
	        }else{
	        	$this->db->where('maintenance_id',$post['maintenance_id']);
	        	$res = $this->db->update('asset_maintenance',$post);
				$this->asset_logs($post['maintenance_id'],'Update Maintenance');
				if($res==1){
					$this->session->set_flashdata('success','Maintenance Updated Successfully...');
				}else{
					$this->session->set_flashdata('failed','Maintenance Updated Failed...');
				}
	        }
		}
		redirect('admin/maintenances');
	...
	public function edit_maintenance($maintenance_id='')
	{
		checklogin_admin('Asset Maintenance','Write');
		$admin_id=$this->session->userdata('emp_id');
		$data['maintenance']=$this->db->query("SELECT `maintenance_id`, `asset_id`, `supplier_id`, `type_id`, `start_dt`, `end_dt`, `comments` ,`is_active`, `created_at`, `updated_at` FROM `asset_maintenance` WHERE `maintenance_id`=$maintenance_id")->row_array();
		$data['assets']=$this->db->query("SELECT `asset_id`, `name`, `asset_tag`, `supplier_id` FROM `assets`")->result_array();
		$data['suppliers']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='supplier' AND `is_active`=1")->result_array();
		$data['types']=$this->db->query("SELECT `id`, `name`, `type`, `is_active`, `created_at`, `updated_at` FROM `asset_items` WHERE `type`='type' AND `is_active`=1")->result_array();
	...
	public function change_maintenance_status($maintenance_id='',$sta='')
	{
		checklogin_admin('Asset Maintenance','Write');
	    $this->admin_logs('NULL',$component_id,'Change Asset Maintenance',$sta);
		$data = array('is_active'=>$sta);
		$this->db->where('maintenance_id',$maintenance_id);
		$res=$this->db->update('asset_maintenance',$data);
		if ($res==1)
		{
			$this->session->set_flashdata('success','<div class="alert alert-success msgfade"><strong>Asset maintenance status updated successfully...</strong></div>');
		}
		else
		{
			$this->session->set_flashdata('failed','<div class="alert alert-warning msgfade"><strong>Asset maintenance status update failed!...</strong></div>');
		}
		redirect('admin/maintenances');
	}
	public function candidate_filters()
	{
		checklogin_admin('Asset Maintenance');
		$admin_id=$this->session->userdata('emp_id');
		$data['companys']=$this->db->query("SELECT `company_id`, `company_name` FROM `companies` GROUP BY `company_id`")->result_array();
		$company_id=$this->input->get('company_id');
		if($company_id!=''){
			$data['jobs']=$this->db->query("SELECT `job_id`, `company_id`, `job_title` FROM `jobs` WHERE `company_id`=$company_id GROUP BY `job_title`")->result_array();
		}else{
			$data['jobs']=array();
		}
		$data['company_id']=$this->input->get('company_id');
		$data['job_id']=$this->input->get('job_id');
		$data['sta']=$this->input->get('sta');
		$data['GetRolesAccess']=$this->read_write('Asset');
		$data['active_menu']='maintenances';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/candidate_filters');
		$this->load->view('admin/footer');
	}
	public function get_candidate_filters_list()
	{
		$company_id=$this->input->get('company_id');
		$job_id=$this->input->get('job_id');
		$sta=$this->input->get('sta');
		$start=$this->input->get('start')?intval( $this->input->get('start') ) :0;
		$length=$this->input->get('length')?intval( $this->input->get('length') ) :0;
		$search=$this->input->get('search')?$this->input->get('search'):array('value'=>'');
		$order=$this->input->get('order')?$this->input->get('order') :array(array('column'=>'','dir'=>'DESC'));
		...
		$records=array();
		$returndata=array();
		$totalcount=$this->loginmodel->search_candidate_filters_Details(1,$start,$length,$search['value'],$column,$dir,$company_id,$job_id,$sta);
		$returndata['recordsTotal']=0;
		$returndata['recordsFiltered']=0;
		$returndata['recordsTotal']+=$totalcount;
		if($search['value']!=''){
		...
		$stu['output'] = $this->loginmodel->search_candidate_filters_Details(2,$start,$length,$search['value'],$column,$dir,$company_id,$job_id,$sta);
		$stu['returndata']=$returndata;
		if(is_array($stu['output']) && count($stu['output'])>0){
				foreach($stu['output'] as $k=>$res){
					$NewArr=array();
					$NewArr[]=$k+1;
					$NewArr[]=$res['fname'].' '.$res['lname'];
					$NewArr[]=$res['email_id'];
					$NewArr[]=$res['phone_no'];
					$NewArr[]=$res['job_title'];
					if($res['status']=='internal_interview')
					{
						$Status='interview interview';
					}else if($res['status']=='client_interview')
					{
						$Status='client interview';
					}else{
						$Status=$res['status'];
	...
	public function get_jobs_list()
	{
		...
		$company_id=$this->input->post('company_id');
		if($company_id!=''){
			$get_jobs_list=$this->db->query("SELECT `job_id`, `company_id`, `job_title` FROM `jobs` WHERE `company_id`=$company_id GROUP BY `job_title`")->result_array();
			echo json_encode($get_jobs_list); 
		}else{
			$get_jobs_list=array();
			echo json_encode($get_jobs_list); 
		}
	}
	public function export_candidate_filters($company_id='',$job_id='',$sta='')
	{
		checklogin_admin();
		$admin_id=$this->session->userdata('emp_id');
		$WHere='';
		...
		if($sta!=''){
			if($sta=='source_applied'){
				$WHere.=" AND status='source/applied'";
			}else if($sta=='internal interview'){
				$WHere.=" AND status='internal_interview'";
			}else if($sta=='AND client interview'){
				$WHere.=" AND status='client_interview'";
			}else{
				$WHere.=" AND status='$sta'";
			}
		}
		$dt=$this->db->query("SELECT `candidate_applied_id`, `company_id`, `job_id`, `fname`, `lname`, `email_id`, `phone_no`, `alternative_phone_no`, `married_status`, `gender`, `dob`, `address`, `postcode`, `city`, `state`, `country`, `user_photo_name`, `user_photo_path`, `user_passport_name`, `user_passport_path`, `id_passport_no`, `user_resume_name`, `user_resume_path`, `ctc_from`, `ctc_to`, `excepted_ctc_from`, `excepted_ctc_to`, `work_experience`, `work_link_portfolio`, `notice_period`, `skills`, `rating`, `comments`, `status`, `is_active`, `created_at`, `updated_at` FROM `candidate_applied_jobs` WHERE $WHere")->result_array();
		$date =date('d-M-Y');
		if(!empty($dt)){
			$j2_id=$dt[0]['job_id'];
				if($j2_id!=''){
					$j_name=$this->db->query("SELECT `job_id`, `company_id`, `job_title` FROM `jobs` WHERE `job_id`=$j2_id")->row_array();
					$name = $j_name['job_title'].'_'.$date;
				}
		}else{
			$name = 'Candidate_profiles_'.$date;
		}
		$name=$name.'.xls';
        header("Content-Type: application/vnd.ms-excel");
		header("Content-disposition: Attachment; filename=$name");
	...
	public function view_asset()
	{
		...
		$data['active_menu']='assets';
		$this->load->view('admin/menu',$data);
		$this->load->view('admin/view_asset');
		$this->load->view('admin/footer');
	}
}