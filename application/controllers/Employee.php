    	        	...
    	        	if($leave_type=='Annual Leave'){
        		        	...
        		        	if(($final_leave_days)+($get_applied_annual_leave_count['SUM(`leave_days`)'])<=$annual_count_chk)
        		        	{
    						        ...
    						        if($_FILES['simage']['name'] !='')
    						        {
    						            ...
    						            $config['overwrite']=true;
    						            $this->upload->initialize($config);
    						            $this->check_file_type($file_name,'pdf','leave_doctor_certificate');
    						            if(!$this->upload->do_upload('simage'))
    						            {
...
// 		echo $this->db->last_query();exit;
	...
	public function ajax_save_add_more()
	{
		...
		if(empty($project_name) || empty($Hrs))
		{
			...
			if($Chk==0)
			{
				...
				$UpdateHrsArr=array('worked_hours'=>$HrsCount['Hrs'],'type_of_work_performed'=>$type_of_work_performed);
	...
	public function ajax_save_item_and_remarks()
	{
		checklogin_employee_helper();
		$emp_id=$this->session->userdata('emp_id');
		$type_of_work_performed=$this->input->post('val');
		$type=$this->input->post('type');
		$item=$this->input->post('item');
		$weekstart_enddate=$this->input->post('dt');
		$GetDates = explode('_', $weekstart_enddate);
		$FromDate=$GetDates[0];
		$ToDate=$GetDates[1];
		if($type=='remarks'){
			$UpdateHrsArr=array('comments'=>$type_of_work_performed);
		}else{
			$UpdateHrsArr=array('type_of_work_performed'=>$type_of_work_performed);
		}
		$this->db->where('item',$item);
		$this->db->where('worked_date>=',$FromDate);
		$this->db->where('worked_date<=',$ToDate);
		$this->db->where('emp_id',$emp_id);
		$res=$this->db->update('timesheet_management',$UpdateHrsArr);
		if($res==1){
			echo 1;
	...
	public function claims()
	{
		...
		$this->load->view('employee/menu',$data);
		$this->load->view('employee/claims');
		$this->load->view('employee/footer');
	}
	public function get_claims_list()
	{
		...
		$records=array();
		$returndata=array();
		$totalcount=$this->employeemodel->search_claim_Details(1,$start,$length,$search['value'],$column,$dir);
		$returndata['recordsTotal']=0;
		$returndata['recordsFiltered']=0;
		$returndata['recordsTotal']+=$totalcount;
		if($search['value']!=''){
		...
		$stu['output'] = $this->employeemodel->search_claim_Details(2,$start,$length,$search['value'],$column,$dir);
		$stu['returndata']=$returndata;
		// echo $this->db->last_query();exit;
		if(is_array($stu['output']) && count($stu['output'])>0){
				foreach($stu['output'] as $k=>$res){
	...
	public function add_claim()
	{
		checklogin_employee_helper();
		$emp_id=$this->session->userdata('emp_id');
		$data['claim_type']=$this->db->query("SELECT `claim_type_id`, `name`, `is_active`, `created_at` FROM `claim_type` WHERE `is_active`=1")->result_array();
		$data['active_menu']='claims';
		$this->load->view('employee/menu',$data);
		$this->load->view('employee/add_claim');
		$this->load->view('employee/footer');
	}
	public function save_claim()
	{
		...
		$created_date =date("Y-m-d H:i:s");
		$config = array( 
			array( 'field'   => 'claim_type_id', 'label'   => 'Claim Type','rules'   => 'required'),
        ...
        {	
		    ...
		    $post['date']=YY_MM_DD($post['date']);
		     $post['is_approved']='Pending';
	        $post['created_at']=$created_date;
	        $post['is_active']=1;
	        $post['emp_id']=$emp_id;
			$res=$this->db->insert('claims',$post);
			$insert_id = $this->db->insert_id();
	        foreach ($_FILES['simage']['name'] as $key => $image)
	        {
	            $_FILES['images']['name']= $_FILES['simage']['name'][$key];
	            ...
	            $images[] = $filePath;
	            $config['upload_path'] = 'assets/claim_documents';
            	...
            	if($fileName!='')
            	{
            		$ArrImgInsert =array('claim_id'=>$insert_id,'file_name'=>$fileName,'file_name_path'=>$filePath,'is_active'=>1,'created_at'=>$created_date);
                	$this->db->insert('claim_documents',$ArrImgInsert);
            	}
    		}
			if($res==1){
				$this->session->set_flashdata('success','Claims Added Successfully...');
			}else{
				$this->session->set_flashdata('failed','This Claims Added Failed...');
			}
		}
		redirect('employee/claims');
	}
